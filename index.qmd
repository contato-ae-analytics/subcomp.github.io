...
gerar_dashboard <- function(bd_principal, 
  ...
  plotly::plot_ly(df_modalidade, labels = ~label_grafico, values = ~n_processos, type = 'pie', hole = 0.68,
                  textinfo = 'text', text = ~label_grafico, hoverinfo = 'text', hovertext = ~hover_txt,
                  marker = base::list(colors = cores_fatias, line = base::list(color = '#FFFFFF', width = largura_barra))) %>%
    plotly::layout(margin = base::list(t = 30, b = 30, l = 20, r = 20), font = base::list(family = fonte_padrao, size = tam_fatias),
                   showlegend = FALSE, hoverlabel = base::list(bgcolor = "white", bordercolor = "#ecf0f1", align = "left", namelength = -1, font = base::list(family = fonte_padrao)),
                   annotations = base::list(text = texto_centro, showarrow = FALSE)) %>%
    plotly::config(displayModeBar = FALSE, responsive = TRUE)
}
...
gerar_dashboard(
  ...
  tam_centro_cab   = 12,                # Tamanho do título EFICIÊNCIA
  tam_centro_corpo = 9,                # Tamanho das linhas TOTAL/OK/PEND
