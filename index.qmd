...
tabela_interativa <- function(dados, 
  ...
  if (!base::is.null(largura_cols_vetor)) {
    for (i in base::seq_along(largura_cols_vetor)) {
      # CORREÇÃO DO ERRO '7': O alvo não pode exceder o número de colunas
      if ((i - 1) < n_colunas_reais) {
  ...
  tabela <- DT::datatable(
    ...
    options = base::list(
      dom = 'ft p', # PESQUISA NA ESQUERDA (f na frente do t)
      pageLength = n_pages,
      searching = TRUE, 
      lengthChange = FALSE,
      info = FALSE,
      ...
      initComplete = DT::JS(
        base::paste0("function(settings, json) {",
                     ...
                     # SEU HOVER: Negrito + Cinza + Texto Preto
...
tabela_interativa_detalhes <- function(dados, 
  ...
  if (!base::is.null(largura_cols_vetor)) {
    for (i in 1:base::length(largura_cols_vetor)) {
      if (i < n_colunas_reais) { # CORREÇÃO ERRO '7'
    ...
    options = base::list(
      ...
      initComplete = DT::JS(
        base::paste0("function(settings, json) {",
                     ...
                     # SEU HOVER PROTEGIDO: Só na tabela principal, nunca nos detalhes
                     "$(this.api().table().body()).on('mouseenter', 'tr', function() {",
                     "  var tr = $(this);",
...
bd_central_downloads <- base::data.frame(
  ...
  "url"         = base::c(
    ...
    "https://docs.google.com/spreadsheets/d/1UAgVUdYJmIwt0DHRX7mNhkZKpR4nPGyeiUqHirggZ2I/edit?gid=1061612826", # Itens
...
# Criando o botão de acesso (HTML) - Adicionamos display: inline-block para garantir o formato
bd_central_downloads$botao <- base::apply(bd_central_downloads, 1, function(x) {
  base::paste0("<a href='", x['url'], "' target='_blank' style='background-color:#03045e; color:white; padding:6px 14px; text-decoration:none; border-radius:4px; font-weight:bold; font-size:10px; display:inline-block;'>PLANILHA</a>")
})

# --- VETORES DE CONFIGURAÇÃO ---
minhas_colunas_down <- base::c("Banco de Dados" = "id_planilha", "Descrição do Conteúdo" = "descricao", "Acesso Direto" = "botao")
minhas_larguras_down <- base::c("180px", "450px", "120px")

# ==========================================================
# 2. EXECUÇÃO DA TABELA (COM CORREÇÃO DE ESCAPE)
# ==========================================================

# Chamamos a função, mas precisamos garantir que o DT não 'limpe' o HTML
# Para isso, vamos passar o parâmetro escape = FALSE direto na chamada se sua função permitir, 
# ou ajustar a função interna.
...
tabela_interativa <- function(dados, 
  ...
  tabela <- DT::datatable(
    base::data.frame(dados_processados, check.names = FALSE),
    escape = FALSE,  # <--- ESSA É A CHAVE PARA O BOTÃO APARECER
    rownames = mostrar_indice,
    # ... (resto das suas opções) ...
  )
  return(htmltools::tagList(html_titulo, tabela))
}
```
