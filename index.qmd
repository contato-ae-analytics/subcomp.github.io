...
# ==============================================================================
# FUNÇÃO FINAL CONSOLIDADA (VERSÃO ALEX) - 100% COMENTADA
# ==============================================================================

tabela_interativa_detalhes <- function(dados, 
                                       dados_detalhes,       # Base com os itens (ex: 722 itens)
                                       coluna_chave,         # Coluna de ligação (ex: "processo")
                                       titulo_tabela,        # Título do relatório
                                       coluna_ordenacao = NULL, # Coluna para ordenar Z-A
                                       ordem_decrescente = TRUE, # Se TRUE, maior para menor
                                       colunas_detalhes_nomes = NULL, # Nomes amigáveis dos itens
                                       largura_detalhes_vetor = NULL, # Larguras px dos itens
                                       tam_fonte_corpo = "11.3px",   # Fonte dados Alex
                                       tam_fonte_header = "12.2px",  # Fonte cabeçalho Alex
                                       tam_fonte_titulo = "14.3px",  # Fonte título Alex
                                       cor_titulo = "#03045e",        # Cor Azul
                                       cor_primaria = "#000000",      # Cor Preta (Destaque)
                                       cor_texto_corpo = "#000000",   # Cor Texto
                                       mostrar_filtro = TRUE,        # Ativa filtros colunas
                                       pesquisa_geral = TRUE,        # Ativa Search global
                                       largura_cols_vetor = NULL,    # Larguras tabela pai
                                       colunas_selecionadas = NULL,  # Mapeamento colunas
                                       n_pages = 10,                 # Linhas por página
                                       mostrar_indice = FALSE) {     # Oculta índice R

  # --- 1. GESTÃO DO CONTADOR ---
  # Se o contador global não existir, inicia com zero
  if (!base::exists("contador_tabelas_alex")) {
    base::assign("contador_tabelas_alex", 0, envir = .GlobalEnv)
  }
  
  # Cria cópia para processar
  dados_processados <- dados

  # --- 2. ORDENAÇÃO (DPLYR) ---
  # Ordena antes de renomear para evitar erro de coluna não encontrada
  ...
  # Aplica o mapeamento 'minhas_colunas'
  if (!base::is.null(colunas_selecionadas)) {
    dados_processados <- dados_processados %>% dplyr::select(tidyselect::all_of(colunas_selecionadas))
  }

  # Adiciona a coluna do botão [+] na posição 0
  dados_processados <- base::cbind(' ' = '&oplus;', dados_processados)
  
  # Incrementa contador e monta título final
  base::assign("contador_tabelas_alex", contador_tabelas_alex + 1, envir = .GlobalEnv)
  titulo_final <- base::paste0("Tabela ", contador_tabelas_alex, ": ", titulo_tabela)

  # --- 4. PREPARAÇÃO JSON ---
  # Converte dados para o navegador
  ...
  tabela <- DT::datatable(
    base::data.frame(dados_processados, check.names = FALSE),
    escape = FALSE, # Renderiza ícones HTML
    rownames = mostrar_indice,
    extensions = 'Buttons',
    filter = if(mostrar_filtro) base::list(position = "top", clear = TRUE, plain = TRUE) else "none",
    options = base::list(
      dom = 'Bft p', # Botões, filtro, tabela, paginação
      buttons = base::list(
        'copy', 'excel', 
      ...
      ordering = FALSE, # Mantém a ordem do R
      autoWidth = FALSE,
      scrollX = TRUE,
      ...
      # --- 6. LÓGICA JAVASCRIPT (CORRIGIDA) ---
      initComplete = DT::JS(
        base::paste0("function(settings, json) {",
                     # Aplica fontes e cores no container
                     "$(this.api().table().container()).css({'font-size': '", tam_fonte_corpo, "', 'font-family': 'Arial, sans-serif'});",
                     # Aplica moldura de 3px no cabeçalho
                     "$(this.api().table().header()).find('tr:first-child th').css({'border-top': '3px solid #000000', 'border-bottom': '3px solid #000000', 'padding': '12px', 'color': '", cor_primaria, "', 'font-size': '", tam_fonte_header, "'});",
                     # Aplica borda no rodapé da tabela
                     "$(this.api().table().node()).css({'border-bottom': '3px solid #000000'});",
                     
                     # LOOP DE FILTROS: Remove busca da col [+] e personaliza o resto
                     ...
                     # EVENTO CLIQUE: Expandir e Travar Negrito
                     ...
                     "    // TRAVA O NEGRITO AO SELECIONAR (Comentário corrigido para JS)",
                     ...
                     # MONTAGEM DA SUBTABELA (DETALHES)
                     ...
                     # EVENTO HOVER: Negrito ao passar o mouse (Só na principal)
...
plotly::plot_ly() %>%
  ...
  plotly::layout(
    ...
    margin = base::list(t = 150, b = 40, l = 30, r = 30),
