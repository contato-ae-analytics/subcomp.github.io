...
#                      "    var html_sub = '<table style=\"width:97%; margin-left:3%; border-collapse: collapse; border: 0px solid #ddd;\"><thead><tr style=\"background:#f9f9f9; color:#03045e; font-weight:bold;\">';",
...
#                      "       html_sub += '<th style=\"padding:8px; border-bottom:0px solid #000; text-align:left; '+width+'\">' + label + '</th>';",
#                      "    });",
...
#                      "});",
#                      
#                      # --- CORREÇÃO DO HOVER (Lógica para evitar bugs em Child Rows) ---
#                      "$(this.api().table().body()).on('mouseenter', 'tr', function() {",
#                      "  if (!$(this).hasClass('shown') && !$(this).parent().is('thead') && !$(this).closest('table').is('.details-control table')) {",
#                      "    $(this).css({'color': '", cor_primaria, "', 'font-weight': '600', 'transition': '0.2s'});",
#                      "  }",
#                      "}).on('mouseleave', 'tr', function() {",
#                      "  $(this).css({'color': '", cor_texto_corpo, "', 'font-weight': 'normal'});",
#                      "});",
#                      "}")
#       ),
#       language = base::list(
...
# ==========================================================
# --- FUNÇÃO: TABELA INTERATIVA COM DESTAQUE PERMANENTE ---
# ==========================================================
...
#                      "$(this.api().table().node()).css({'border-bottom': '3px solid #000000'});",
#                      
#                      # Lógica de expansão com destaque permanente (CSS Inline)
...
#                      "  if (row.child.isShown()) {",
#                      "    row.child.hide(); tr.removeClass('shown'); $(this).html('&oplus;');",
#                      # REMOVE O DESTAQUE AO FECHAR
#                      "    tr.css({'color': '", cor_texto_corpo, "', 'font-weight': 'normal'});",
#                      "  } else {",
#                      # APLICA O DESTAQUE PERMANENTE AO ABRIR
#                      "    tr.css({'color': '", cor_primaria, "', 'font-weight': '800'});",
#                      "    var rowData = row.data();",
#                      "    var chave = rowData[", if(mostrar_indice) "2" else "1", "];",
#                      "    var sub_dados = detalhes_data.filter(d => d['", coluna_chave, "'] == chave);",
#                      "    var html_sub = '<table class=\"subtable\" style=\"width:97%; margin-left:3%; border-collapse: collapse;\"><thead><tr style=\"background:#f9f9f9; color:#03045e; font-weight:bold;\">';",
#                      "    var colKeys = Object.keys(sub_dados[0]);",
#                      "    colKeys.forEach((key, index) => {",
#                      "       var label = (customNomes && customNomes[index]) ? customNomes[index] : key;",
#                      "       var width = (customLarguras && customLarguras[index]) ? 'width:'+customLarguras[index]+';' : '';",
#                      "       html_sub += '<th style=\"padding:4px; border-bottom:1px solid #ddd; text-align:left; '+width+'\">' + label + '</th>';",
#                      "    });",
#                      "    html_sub += '</tr></thead><tbody>';",
#                      "    sub_dados.forEach(r => {",
#                      "       html_sub += '<tr style=\"border-bottom:1px solid #eee; font-weight:normal !important; color:", cor_texto_corpo, " !important;\">';",
#                      "       colKeys.forEach(k => { html_sub += '<td style=\"padding:8px;\">' + r[k] + '</td>'; });",
#                      "       html_sub += '</tr>';",
#                      "    });",
#                      "    html_sub += '</tbody></table>';",
...
#                      "});",
#                      
#                      # HOVER APENAS PARA LINHAS NÃO ABERTAS E APENAS NA TABELA PAI
#                      "$(this.api().table().body()).on('mouseenter', 'tr', function() {",
#                      "  if (!$(this).hasClass('shown') && !$(this).closest('table').hasClass('subtable')) {",
#                      "    $(this).css({'color': '", cor_primaria, "', 'font-weight': '800', 'transition': '0.1s'});",
#                      "  }",
#                      "}).on('mouseleave', 'tr', function() {",
#                      "  if (!$(this).hasClass('shown') && !$(this).closest('table').hasClass('subtable')) {",
#                      "    $(this).css({'color': '", cor_texto_corpo, "', 'font-weight': 'normal'});",
...
# ==========================================================
# --- FUNÇÃO: TABELA INTERATIVA (CORREÇÃO DE VARIÁVEL JS) ---
# ==========================================================
...
#   )
# 
#   # --- 2. PREPARAÇÃO JSON PARA JAVASCRIPT (CORREÇÃO AQUI) ---
#   json_detalhes <- jsonlite::toJSON(dados_detalhes)
#   
#   nomes_detalhes_js <- if(!base::is.null(colunas_detalhes_nomes)) {
#     jsonlite::toJSON(colunas_detalhes_nomes)
...
#   }
#   
#   # Variável que estava faltando na versão anterior
#   larguras_detalhes_js <- if(!base::is.null(largura_detalhes_vetor)) {
#     jsonlite::toJSON(largura_detalhes_vetor)
#   } else {
#     "null"
...
#                      "  if (row.child.isShown()) {",
#                      "    row.child.hide(); tr.removeClass('shown'); $(this).html('&oplus;');",
#                      "    tr.css({'background-color': '', 'color': '", cor_texto_corpo, "', 'font-weight': 'normal', 'border-left': 'none'});",
#                      "  } else {",
...
#                      "});",
#                      
#                      # Hover Protegido (Só na tabela pai e apenas se não estiver aberta)
#                      "$(this.api().table().body()).on('mouseenter', 'tr', function() {",
#                      "  var tr = $(this);",
#                      "  if (!tr.hasClass('shown') && !tr.closest('table').hasClass('subtable')) {",
#                      "    tr.css({'background-color': '#f1f1f1', 'transition': '0.1s'});",
#                      "  }",
#                      "}).on('mouseleave', 'tr', function() {",
#                      "  var tr = $(this);",
#                      "  if (!tr.hasClass('shown') && !tr.closest('table').hasClass('subtable')) {",
...
# 
# ==========================================================
# --- FUNÇÃO: TABELA INTERATIVA (HOVER + SELEÇÃO FIXA) ---
# ==========================================================
    ...
    options = base::list(
      ...
      initComplete = DT::JS(
        base::paste0("function(settings, json) {",
                     ...
                     # HOVER APENAS NA TABELA PRINCIPAL (NÃO NA SUBTABLE)
...
tabela_interativa_detalhes(
  ...
  # --- NOVOS COMANDOS DO ALEX (ORDENAÇÃO) ---
  coluna_ordenacao = "processo",  # Define qual variável dita a ordem (ex: processo, agente ou valor)
  ordem_decrescente = TRUE,       # TRUE para Maior -> Menor (Z-A) | FALSE para A-Z
