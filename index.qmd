...
# ==============================================================================
# E. FUNÇÕES DE APRESENTAÇÃO - CORRIGIDAS
# ==============================================================================

# ==============================================================================
# E. FUNÇÕES DE APRESENTAÇÃO - ESTILO ORIGINAL ALEX
# ==============================================================================

# ==============================================================================
# E. FUNÇÕES DE APRESENTAÇÃO - VERSÃO FINAL ALEX (CORRIGIDA)
# ==============================================================================
# ==============================================================================
# E. FUNÇÕES DE APRESENTAÇÃO - VERSÃO FINAL ALEX (SEM ERROS)
# ==============================================================================
# ==============================================================================
# E. FUNÇÕES DE APRESENTAÇÃO - VERSÃO INTEGRAL COMENTADA LINHA POR LINHA
# ==============================================================================

# Definição da função de tabela simples com todos os seus argumentos de entrada
tabela_interativa <- function(dados,                # Dataframe com os dados brutos
                              titulo_tabela,       # Texto que aparecerá no título H2
                              tam_fonte_corpo = "13px",   # Tamanho da fonte das células
                              tam_fonte_titulo = "20px",  # Tamanho da fonte do título H2
                              tam_fonte_header = "14px",  # Tamanho da fonte do cabeçalho
                              cor_titulo = "#03045e",     # Cor oficial do título (Marinho)
                              cor_primaria = "#03045e",   # Cor de destaque do cabeçalho
                              cor_texto_corpo = "#333333",# Cor do texto padrão das linhas
                              mostrar_filtro = TRUE,      # Ativa/desativa filtros por coluna
                              pesquisa_geral = FALSE,     # Ativa/desativa campo de busca global
                              largura_cols_vetor = NULL,  # Vetor com larguras em px
                              colunas_selecionadas = NULL,# Vetor com mapeamento de colunas
                              n_pages = 10,               # Linhas visíveis por página
                              mostrar_indice = TRUE) {    # Exibe ou oculta números das linhas

  # Verifica se o contador de tabelas existe no ambiente global do R
  if (!base::exists("contador_tabelas_alex")) {
    # Cria o contador iniciando em zero se for a primeira tabela
    base::assign("contador_tabelas_alex", 0, envir = .GlobalEnv)
  }

  # Atribui os dados recebidos para uma variável de processamento
  dados_processados <- dados
  # Checa se o usuário enviou um vetor para selecionar/renomear colunas
  if (!base::is.null(colunas_selecionadas)) {
    # Filtra o dataframe usando o mapeamento fornecido via dplyr
    dados_processados <- dados %>% dplyr::select(tidyselect::all_of(colunas_selecionadas))
  }

  # Obtém o total de colunas para evitar erros de índice (Erro '7')
  n_colunas_dados <- base::ncol(dados_processados)
  # Inicia a lista de larguras com a coluna de índice (target 0) fixa em 30px
  lista_columnDefs <- base::list(base::list(width = '30px', targets = 0))
  
  # Se o vetor de larguras existir, percorre os valores para aplicar às colunas
  if (!base::is.null(largura_cols_vetor)) {
    # Inicia o laço de repetição sobre o vetor de larguras
    for (i in base::seq_along(largura_cols_vetor)) {
      # Garante que não tentaremos aplicar largura em coluna inexistente
      if ((i - 1) < n_colunas_dados) {
         # Adiciona a definição de largura para a coluna alvo na lista técnica
  ...
  # Incrementa o contador global para que cada tabela receba um número sequencial
  base::assign("contador_tabelas_alex", base::get("contador_tabelas_alex", envir = .GlobalEnv) + 1, envir = .GlobalEnv)
  # Formata o texto final do título incluindo o número atual do contador
  titulo_final <- base::paste0("Tabela ", base::get("contador_tabelas_alex", envir = .GlobalEnv), ": ", titulo_tabela)

  # Constrói o cabeçalho H2 utilizando tags HTML do pacote htmltools
  html_titulo <- htmltools::tags$h2(
    # Aplica o CSS com as variáveis de cor e tamanho solicitadas por você
    style = base::paste0("color: ", cor_titulo, "; font-size: ", tam_fonte_titulo, 
                         "; font-weight: bold; text-align: left; margin-bottom: 10px;"),
    # Define o texto do título dentro da tag de negrito (b)
    htmltools::tags$b(titulo_final)
  )

  # Inicia a criação da tabela interativa através do pacote DT (DataTables)
  tabela <- DT::datatable(
    # Converte para dataframe garantindo integridade dos nomes das colunas
    base::data.frame(dados_processados, check.names = FALSE),
    # escape = FALSE permite que links e botões HTML funcionem (não sejam texto puro)
    escape = FALSE, 
    # Determina se os números laterais das linhas serão exibidos
    rownames = mostrar_indice,
    # Configura o filtro superior das colunas (estilo Alex)
    filter = if(mostrar_filtro) base::list(position = "top", clear = TRUE, plain = TRUE) else "none",
    # Define que a tabela ocupe 100% da largura da página
    width = "100%", 
    # Aplica classes CSS para realce de linha e bordas horizontais simples
    class = "hover row-border", 
    # Inicia a lista de opções técnicas de controle do plugin DataTables
    options = base::list(
      # dom = 'ft p' fixa a barra de pesquisa na ESQUERDA (f) antes da tabela (t)
      dom = 'ft p', 
      # Define a quantidade inicial de linhas mostradas por página
      pageLength = n_pages,
      # Habilita o campo de busca/pesquisa geral
      searching = TRUE, 
      # Desativa a opção do usuário alterar o número de registros por página
      lengthChange = FALSE,
      # Oculta a frase informativa "Mostrando X de Y" do rodapé
      info = FALSE,
      # Desativa a ordenação padrão por clique nas colunas (estilo Alex)
      ordering = FALSE,      
      # Desativa o redimensionamento automático para usar as larguras fixas
      autoWidth = FALSE,    
      # Ativa a barra de rolagem horizontal para evitar quebra de layout
      scrollX = TRUE,       
      # Injeta as definições de largura de colunas calculadas anteriormente
      columnDefs = lista_columnDefs,
      # Define código JavaScript para ser executado ao finalizar o carregamento
      initComplete = DT::JS(
        base::paste0("function(settings, json) {",
                     # Aplica fonte, família e cor no container principal da tabela
                     "$(this.api().table().container()).css({'font-size': '", tam_fonte_corpo, "', 'font-family': 'Arial, sans-serif', 'color': '", cor_texto_corpo, "'});",
                     # Estiliza o cabeçalho com fundo branco e cor de destaqueAlex
                     "$(this.api().table().header()).css({'font-size': '", tam_fonte_header, "', 'background-color': '#ffffff', 'color': '", cor_primaria, "', 'white-space': 'nowrap'});",
                     # Cria a borda preta grossa de 3px no topo e na base do cabeçalho
                     "$(this.api().table().header()).find('tr:first-child th').css({'border-top': '3px solid #000000', 'border-bottom': '3px solid #000000', 'padding': '12px'});",
                     # Cria a borda preta grossa de 3px no encerramento da tabela
                     "$(this.api().table().node()).css({'border-bottom': '3px solid #000000'});",
                     # Monitora quando o mouse passa sobre uma linha da tabela
                     "$(this.api().table().body()).on('mouseenter', 'tr', function() {",
                     # Garante que o hover só funcione na tabela pai (bloqueia sub-tabelas)
                     "  if (!$(this).closest('table').hasClass('subtable')) {",
                     # Aplica fundo cinza, texto preto e negrito no hover (Original Alex)
                     "    $(this).css({'background-color': '#f1f1f1', 'color': '#000000', 'font-weight': 'bold'});",
                     "  }",
                     # Monitora quando o mouse sai da linha da tabela
                     "}).on('mouseleave', 'tr', function() {",
                     # Verifica se não é sub-tabela para limpar os estilos aplicados
                     "  if (!$(this).closest('table').hasClass('subtable')) {",
                     # Remove o fundo e volta o peso da fonte para o normal
                     "    $(this).css({'background-color': '', 'color': '", cor_texto_corpo, "', 'font-weight': 'normal'});",
                     "  }",
                     "});",
                     "}")
      ),
      # Define as traduções para os textos de paginação e avisos do DataTables
  ...
  # Retorna o título HTML e a tabela como um conjunto pronto para renderização
  return(htmltools::tagList(html_titulo, tabela))
}


# Definição da função hierárquica (Pai e Filho) com todos os argumentos comentados
tabela_interativa_detalhes <- function(dados,               # Dataframe principal (Pai)
                                       dados_detalhes,      # Dataframe secundário (Filho)
                                       coluna_chave,        # Nome da coluna que liga as duas bases
                                       titulo_tabela,       # Título que aparecerá no topo
                                       colunas_detalhes_nomes = NULL, # Nomes amigáveis dos detalhes
                                       largura_detalhes_vetor = NULL, # Larguras px da sub-tabela
                                       tam_fonte_corpo = "13px",   # Tamanho da fonte do corpo
                                       tam_fonte_titulo = "20px",  # Tamanho da fonte do título
                                       tam_fonte_header = "14px",  # Tamanho da fonte do cabeçalho
                                       cor_titulo = "#03045e",     # Cor marinho do título
                                       cor_primaria = "#03045e",   # Cor marinho do cabeçalho
                                       cor_texto_corpo = "#333333",# Cor cinza do texto
                                       mostrar_filtro = TRUE,      # Ativa filtros individuais
                                       pesquisa_geral = FALSE,     # Ativa barra de busca global
                                       largura_cols_vetor = NULL,  # Larguras px da tabela pai
                                       colunas_selecionadas = NULL,# Seleção de colunas pai
                                       n_pages = 10,               # Registros por página
                                       mostrar_indice = TRUE) {    # Exibe ou não o índice numérico

  # Verifica e inicializa o contador de tabelas no ambiente global
  if (!base::exists("contador_tabelas_alex", envir = .GlobalEnv)) {
    # Define como zero caso não exista
    base::assign("contador_tabelas_alex", 0, envir = .GlobalEnv)
  }

  # Prepara os dados da tabela pai
  dados_processados <- dados
  # Se houver vetor de colunas, seleciona via dplyr::select Alex
  if (!base::is.null(colunas_selecionadas)) {
    # Aplica filtro conforme mapeamento de colunas enviado
    dados_processados <- dados %>% dplyr::select(tidyselect::all_of(colunas_selecionadas))
  }

  # Cria a coluna com o botão de expansão [+] na primeira posição lateral
  dados_processados <- base::cbind(' ' = '&oplus;', dados_processados)
  # Obtém número total de colunas para cálculo de larguras
  n_colunas_reais <- base::ncol(dados_processados)

  # Atualiza o contador de tabelas do relatório
  base::assign("contador_tabelas_alex", base::get("contador_tabelas_alex", envir = .GlobalEnv) + 1, envir = .GlobalEnv)
  # Gera o texto do título numerado sequencialmente
  titulo_final <- base::paste0("Tabela ", base::get("contador_tabelas_alex", envir = .GlobalEnv), ": ", titulo_tabela)

  # Constrói o cabeçalho H2 Alex em HTML
  html_titulo <- htmltools::tags$h2(
    # Aplica estilos de fonte, cor e alinhamento
    style = base::paste0("color: ", cor_titulo, "; font-size: ", tam_fonte_titulo, 
                         "; font-weight: bold; text-align: left; margin-bottom: 10px;"),
    # Texto do título em negrito
    htmltools::tags$b(titulo_final)
  )

  # Serializa os dados de detalhes para JSON para processamento no navegador
  json_detalhes_js <- jsonlite::toJSON(dados_detalhes, ArrayOrder = "row")
  # Serializa os nomes das colunas de detalhes para JSON (ou null)
  nomes_detalhes_js <- if(!base::is.null(colunas_detalhes_nomes)) jsonlite::toJSON(colunas_detalhes_nomes) else "null"
  # Serializa as larguras da sub-tabela para JSON (ou null)
  larguras_detalhes_js <- if(!base::is.null(largura_detalhes_vetor)) jsonlite::toJSON(largura_detalhes_vetor) else "null"

  # Inicia configuração de colunas pai, definindo a coluna 0 como classe details-control
  lista_column_defs <- base::list(
    base::list(targets = 0, className = 'details-control', width = '20px', orderable = FALSE)
  )
  
  # Aplica as larguras nas colunas principais sem exceder o limite real (evita erro '7')
  if (!base::is.null(largura_cols_vetor)) {
    for (i in 1:base::length(largura_cols_vetor)) {
      if (i < n_colunas_reais) {
        # Adiciona a largura fixa na lista técnica do DataTables
        lista_column_defs[[base::length(lista_column_defs) + 1]] <- base::list(targets = i, width = largura_cols_vetor[i])
      }
    }
  }

  # Renderiza a tabela principal com suporte a expansão
  tabela <- DT::datatable(
    # Envia os dados com a coluna de controle inclusa
    base::data.frame(dados_processados, check.names = FALSE),
    # escape = FALSE é obrigatório para renderizar o ícone HTML [+]
    escape = FALSE, 
    # Controla exibição dos números das linhas
    rownames = mostrar_indice,
    # Define os filtros de coluna no topo
    filter = if(mostrar_filtro) base::list(position = "top", clear = TRUE, plain = TRUE) else "none",
    # Define ocupação total da largura
    width = "100%", 
    # Aplica bordas simples entre registros
    class = "row-border", 
    # Inicia definições de comportamento DataTables
    options = base::list(
      # dom = 'ft p' mantém campo de BUSCA na ESQUERDA
      dom = 'ft p', 
      # Linhas por página
      pageLength = n_pages,
      # Ativa campo de busca global
      searching = TRUE, 
      # Trava a quantidade de registros por página
      lengthChange = FALSE,
      # Remove sumário de informações inferior
      info = FALSE,
      # Remove ordenação padrão
      ordering = FALSE,      
      # Trava larguras automáticas
      autoWidth = FALSE,    
      # Habilita rolagem lateral
      scrollX = TRUE,
      # Aplica a lista de larguras configurada
      columnDefs = lista_column_defs,
      
      # Código JavaScript para gerenciar a lógica de expansão
      initComplete = DT::JS(
        base::paste0("function(settings, json) {",
                     # CSS via JS: Aplica fontes, cabeçalho e bordas grossas 3px
                     "$(this.api().table().container()).css({'font-size': '", tam_fonte_corpo, "', 'font-family': 'Arial, sans-serif', 'color': '", cor_texto_corpo, "'});",
                     "$(this.api().table().header()).css({'font-size': '", tam_fonte_header, "', 'background-color': '#ffffff', 'color': '", cor_primaria, "', 'white-space': 'nowrap'});",
                     "$(this.api().table().header()).find('tr:first-child th').css({'border-top': '3px solid #000000', 'border-bottom': '3px solid #000000', 'padding': '12px'});",
                     "$(this.api().table().node()).css({'border-bottom': '3px solid #000000'});",
                     
                     # Instancia API e dados JSON em variáveis de script
                     ...
                     "       sub_dados.forEach(r => {",
                     "          html_sub += '<tr style=\"border-bottom:1px solid #f1f1f1;\">';",
                     "          colKeys.forEach(k => { html_sub += '<td style=\"padding:6px; color:#333 !important; background-color: transparent !important;\">' + r[k] + '</td>'; });",
                     "          html_sub += '</tr>';",
                     "       });",
                     "    } else { html_sub += '<tr><td>Nenhum detalhe disponível</td></tr>'; }",
                     "    html_sub += '</tbody></table>';",
                     # Mostra a sub-tabela e altera ícone para [-]
                     "    row.child(html_sub).show(); tr.addClass('shown'); $(this).html('&ominus;');",
                     "  }",
                     "});",

                     # Hover Alex: Garante que efeito não atinja sub-tabelas
                     "$(this.api().table().body()).on('mouseenter', 'tr', function() {",
                     "  var tr = $(this);",
                     "  if (!tr.hasClass('shown') && !tr.closest('table').hasClass('subtable')) {",
                     "    tr.css({'background-color': '#f1f1f1', 'color': '#000000', 'font-weight': 'bold'});",
                     "  }",
                     "}).on('mouseleave', 'tr', function() {",
                     "  var tr = $(this);",
                     "  if (!tr.hasClass('shown') && !tr.closest('table').hasClass('subtable')) {",
      ...
      # Tradução para interface do usuário em Português
      language = base::list(paginate = base::list("previous" = "Anterior", "next" = "Próximo"),
                            emptyTable = "Sem dados", zeroRecords = "Nenhum registro encontrado", search = "Pesquisa Geral:")
    )
  )
  # Retorna o tagList com título e tabela hierárquica
  return(htmltools::tagList(html_titulo, tabela))
}
...
::: {style="font-size: 0.9em; text-align: justify; border-left: 4px solid #000000; padding-left: 15px; margin-top: 15px; margin-bottom: 20px; "}
...
# Criando o botão HTML (Style Alex)
bd_central_downloads$botao <- base::apply(bd_central_downloads, 1, function(x) {
  base::paste0("<a href='", x['url'], "' target='_blank' style='background-color:#03045e; color:white; padding:5px 12px; text-decoration:none; border-radius:4px; font-weight:bold; font-size:10px; display:inline-block;'>PLANILHA</a>")
})

# Vetores
minhas_colunas_down <- base::c("Banco de Dados" = "id_planilha", "Descrição" = "descricao", "Acesso" = "botao")
minhas_larguras_down <- base::c("180px", "450px", "120px")

# Execução
tabela_interativa(
  dados = bd_central_downloads,
  titulo_tabela = "Central de Downloads",
...
plotly::plot_ly() %>%
  ...
  plotly::layout(
    ...
    margin = base::list(t = 150, b = 90, l = 0, r = 0),
...
# Criamos um agrupamento para que cada processo ocupe apenas uma linha na visão inicial
bd_resumo_processos <- bd_principal %>%
  dplyr::group_by(processo) %>% # Agrupa pelo número do processo
  dplyr::summarise(
    # Captura a primeira ocorrência de cada informação fixa do processo
    modalidade = dplyr::first(modalidade_processo),
    agente     = dplyr::first(agente),
    # Soma a quantidade de itens informada na coluna do banco
    total_itens = base::max(qtd_itens_processo, na.rm = TRUE),
    # Soma o valor financeiro total de todos os itens do processo
    v_total_num = base::sum(valor_total, na.rm = TRUE),
    status     = dplyr::first(status_item),
    .groups = "drop" # Finaliza o agrupamento
  ) %>%
  dplyr::mutate(
    # Formata o valor numérico para texto com padrão brasileiro R$
...
# Esta base contém os 722 itens e será filtrada pelo JavaScript ao clicar no "+"
bd_detalhes_itens <- bd_principal %>%
  dplyr::mutate(
    # Formata o valor unitário e total dos itens para o balão de expansão
  ...
  dplyr::select(
    # Mantemos a coluna processo como CHAVE para o vínculo entre as tabelas
    processo, 
    ID = id, 
    Descrição = catalogo_descricao, 
    Fornecedor = fornecedor, 
    Qtd = qtde, 
    `V. Unitário` = v_unit_txt, 
    `Total Item` = v_total_txt
  )
...
# ==========================================================
# 1. VETORES DE CONFIGURAÇÃO: PROCESSOS (PAI E FILHO)
# ==========================================================
...
minhas_colunas <- base::c(
  ...
  "Agente"      = "agente",
  "Itens"       = "total_itens",
  "Valor Total" = "valor_total_proc",
  "Status"      = "status"
)

# Larguras da Tabela Pai (Botão [+] + as 6 colunas acima)
minhas_larguras <- base::c("30px", "150px", "100px", "150px", "80px", "140px", "100px")
...
# Larguras da Sub-tabela
minhas_larguras_detalhes <- base::c("100px", "60px", "400px", "200px", "50px", "100px", "100px")

# ==========================================================
# 2. EXECUÇÃO: TABELA PROCESSOS (Função com Detalhes)
# ==========================================================
tabela_interativa_detalhes(
  dados                  = bd_resumo_processos,
  dados_detalhes         = bd_detalhes_itens,
  coluna_chave           = "processo",
  titulo_tabela          = "Gestão de Processos e Itens",
  
  # Estilo Alex (Usando as variáveis globais de fonte/cor)
  ...
  # Aplicação dos vetores definidos acima
