...
# ==========================================================
# Agrupamento base por modalidade
...
# Criando a linha de TOTAL GERAL (Consolidação de todas as modalidades)
bd_total_geral <- bd_pai_modalidade_base %>%
  dplyr::summarise(
    mod_proc         = base::as.factor("TOTAL GERAL"),
...
# União final com cálculo de percentuais da Tabela Pai
bd_pai_modalidade <- base::rbind(bd_total_geral, bd_pai_modalidade_base) %>%
  dplyr::mutate(
    p_represent_geral = (n_processos / base::max(n_processos[mod_proc == "TOTAL GERAL"])) * 100,
    p_represent_txt   = base::paste0(base::round(p_represent_geral, 1), "%"),
    p_resolvido       = (n_resolvidos / n_processos) * 100,
    aporte_total      = base::paste0("R$ ", base::format(v_total_mod, big.mark = ".", decimal.mark = ",")),
    aporte_resolvido  = base::paste0("R$ ", base::format(v_resolvido, big.mark = ".", decimal.mark = ",")),
    aporte_pendente   = base::paste0("R$ ", base::format(v_pendente, big.mark = ".", decimal.mark = ",")),
  ...
  dplyr::arrange(base::desc(mod_proc == "TOTAL GERAL"), base::desc(p_resolvido))

# ==========================================================
# 3. BANCO FILHO: PERFORMANCE E COMPARAÇÃO POR AGENTE
# ==========================================================
# Detalhes individuais por modalidade
...
# Detalhes específicos para a linha "TOTAL GERAL"
detalhes_total_geral <- status_por_processo %>%
  ...
  dplyr::mutate(
    n_total_geral  = base::sum(total_proc_ag),
    p_particip_txt = base::paste0(base::round((total_proc_ag / n_total_geral) * 100, 1), "%"),
...
# Unindo tudo no Banco Filho
bd_filho_agentes <- dplyr::bind_rows(bd_filho_base, detalhes_total_geral) %>%
  dplyr::select(mod_proc, agente_rank, p_particip_txt, total_proc_ag, res_ag, pen_ag, aporte_gestao, ciclo_ag_txt, media_setor, avaliacao_txt, eficiencia_ag)

# ==========================================================
# 4. CONFIGURAÇÃO DE COLUNAS E EXECUÇÃO
# ==========================================================

# Nomes Tabela Pai (10 colunas)
nomes_col <- base::c(
  "Modalidade"               = "mod_proc",
  "Representatividade<br>(%)" = "p_represent_txt",
  "Nº<br>Processos"          = "n_processos",
  "Valor<br>Consolidado"     = "aporte_total",
  "Processos<br>Finalizados" = "n_resolvidos",
  "Valor<br>Empenhado"       = "aporte_resolvido",
...
largura_col <- base::c("160px", "100px", "80px", "350px", "90px", "350px", "80px", "90px", "350px", "160px")
alinhamento_col <- base::rep("left", 10)

# Nomes Tabela Filho (10 colunas)
nomes_det <- base::c(
  "Agente<br>(Rank)", 
  "Participação<br>(%)",
  "Qtd. Processos", 
