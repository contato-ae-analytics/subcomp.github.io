...
# ==============================================================================
# FUNÇÃO: tabela_interativa_detalhes
# DESENVOLVIDA PARA: Alexssandro da Silva Oliveira (Alex)
# OBJETIVO: Tabela hierárquica (Pai/Filho) com destaque fixo e filtros inteligentes
# ==============================================================================

tabela_interativa_detalhes <- function(dados, 
                                       dados_detalhes,       # Base com os 722 itens (Sub-tabela)
                                       coluna_chave,         # Nome da coluna que liga as duas bases (ex: "processo")
                                       titulo_tabela,        # Texto do cabeçalho
                                       coluna_ordenacao = NULL, # Coluna para o sorteio inicial (Z-A)
                                       ordem_decrescente = TRUE, # Se TRUE, ordena do maior para o menor
                                       colunas_detalhes_nomes = NULL, # Vetor com nomes amigáveis para os itens
                                       largura_detalhes_vetor = NULL, # Vetor com larguras (px) para os itens
                                       tam_fonte_corpo = "11.3px",   # Fonte compacta Alex
                                       tam_fonte_header = "12.2px",  # Fonte cabeçalho Alex
                                       tam_fonte_titulo = "14.3px",  # Fonte título Alex
                                       cor_titulo = "#03045e",        # Azul padrão
                                       cor_primaria = "#000000",      # Preto para molduras e seleção
                                       cor_texto_corpo = "#000000",   # Texto padrão
                                       mostrar_filtro = TRUE,        # Filtros por coluna
                                       pesquisa_geral = TRUE,        # Barra Search global
                                       largura_cols_vetor = NULL,    # Larguras da tabela principal
                                       colunas_selecionadas = NULL,  # Mapeamento de colunas principais
                                       n_pages = 10,                 # Linhas por página
                                       mostrar_indice = FALSE) {     # Oculta números de linha do R

  # --- A. CONTROLE DE AMBIENTE ---
  # Garante que o contador de tabelas persista entre as chamadas no relatório
  if (!base::exists("contador_tabelas_alex")) {
    base::assign("contador_tabelas_alex", 0, envir = .GlobalEnv)
  }
  
  # --- B. PROCESSAMENTO DOS DADOS (R) ---
  dados_processados <- dados
  
  # Filtra e renomeia as colunas conforme o mapeamento do Alex
  if (!base::is.null(colunas_selecionadas)) {
    dados_processados <- dados_processados %>% 
      dplyr::select(tidyselect::all_of(colunas_selecionadas))
  }
  
  # Aplica a ordenação inicial (ex: Processo do maior para o menor)
  if (!base::is.null(coluna_ordenacao)) {
    if (ordem_decrescente) {
      dados_processados <- dados_processados %>% 
        dplyr::arrange(dplyr::desc(.data[[coluna_ordenacao]]))
    } else {
      dados_processados <- dados_processados %>% 
        dplyr::arrange(.data[[coluna_ordenacao]])
    }
  }

  # Adiciona a coluna vazia na posição 0 para o botão [+] de expansão
  dados_processados <- base::cbind(' ' = '&oplus;', dados_processados)
  
  # Atualiza o contador de tabelas para o título
  base::assign("contador_tabelas_alex", contador_tabelas_alex + 1, envir = .GlobalEnv)
  titulo_final <- base::paste0("Tabela ", contador_tabelas_alex, ": ", titulo_tabela)

  # --- C. PREPARAÇÃO PARA O JAVASCRIPT ---
  # Converte os dataframes e vetores para formato JSON (que o navegador entende)
  ...
  # --- D. CONSTRUÇÃO DA INTERFACE (DT) ---
  tabela <- DT::datatable(
    base::data.frame(dados_processados, check.names = FALSE),
    escape = FALSE,    # Permite símbolos HTML como o &oplus;
    rownames = mostrar_indice,
    extensions = 'Buttons', # Habilita botões de exportação
    filter = if(mostrar_filtro) base::list(position = "top", clear = TRUE, plain = TRUE) else "none",
    options = base::list(
      dom = 'Bft p',   # B: Botões, f: Busca, t: Tabela, p: Paginação
      buttons = base::list(
        'copy', 'excel', 
        base::list(extend = 'pdfHtml5', title = titulo_final, orientation = 'landscape')
      ),
      pageLength = n_pages,
      searching = pesquisa_geral,
      ordering = FALSE, # Mantém a ordem definida no R
      autoWidth = FALSE,
      scrollX = TRUE,
      # Configura a primeira coluna (Botão) para ser pequena e centralizada
      columnDefs = base::list(
        base::list(targets = 0, className = 'details-control', width = '30px')
      ),
      
      # --- E. LÓGICA JAVASCRIPT CUSTOMIZADA (O "CÉREBRO" DA TABELA) ---
      initComplete = DT::JS(
        base::paste0("function(settings, json) {",
                     # 1. ESTILIZAÇÃO GERAL (Layout Alex)
                     ...
                     "  'padding': '12px', 'color': '", cor_primaria, "', 'font-size': '", tam_fonte_header, "'",
                     "});",
                     "$(this.api().table().node()).css({'border-bottom': '3px solid #000000'});",
                     
                     # 2. FILTROS INTELIGENTES (Limpa a coluna [+] e coloca placeholders)
                     "this.api().columns().every(function(index) {",
                     "   var column = this;",
                     "   if (index === 0) { $(column.filter()).empty(); } else {",
                     "      var title = $(column.header()).text();",
                     "      if (title.trim() !== '') {",
                     "         $('input', column.filter()).attr('placeholder', 'Busca ' + title.trim());",
                     "         $('input', column.filter()).css({'font-size': '10px', 'width': '100%'});",
                     "      }",
                     "   }",
                     "});",

                     # 3. LÓGICA DE EXPANSÃO (CLIQUE NO [+])
                     ...
                     "    tr.css({'background-color': '', 'font-weight': 'normal', 'color': '", cor_texto_corpo, "'});",
                     "  } else {",
                     "    tr.addClass('shown'); $(this).html('&ominus;');",
                     "    tr.css({'background-color': '#f8f9fa', 'font-weight': 'bold', 'color': '", cor_primaria, "'});",
                     ...
                     # Montagem do HTML da Sub-tabela (Detalhes)
                     "    var html_sub = '<table class=\"subtable_alex\" style=\"width:97%; margin-left:3%; border-collapse: collapse;\"><thead><tr style=\"background:#ffffff; color:#03045e; font-weight:bold;\">';",
                     ...
                     "    sub_dados.forEach(r => {",
                     "       html_sub += '<tr>';",
                     "       colKeys.forEach(k => { html_sub += '<td style=\"padding:6px; background-color: transparent !important; font-weight: normal !important; color: #333 !important;\">' + r[k] + '</td>'; });",
                     "       html_sub += '</tr>';",
                     "    });",
                     "    html_sub += '</tbody></table>';",
                     "    row.child(html_sub).show();",
                     "  }",
                     "});",
                     
                     # 4. HOVER DINÂMICO (Somente na tabela principal e linhas não abertas)
                     "$(this.api().table().node()).on('mouseenter', 'tr', function() {",
                     "  var tr = $(this);",
                     "  if (!tr.closest('table').hasClass('subtable_alex')) {",
                     "    tr.css({'font-weight': 'bold', 'background-color': '#f1f1f1'});",
                     "  }",
                     "}).on('mouseleave', 'tr', function() {",
                     "  var tr = $(this);",
                     "  if (!tr.closest('table').hasClass('subtable_alex') && !tr.hasClass('shown')) {",
                     "    tr.css({'font-weight': 'normal', 'background-color': ''});",
                     "  }",
                     "});",
                     "}")
      ),
  ...
  # Retorno HTML: Título + Tabela
  return(htmltools::tagList(
    htmltools::tags$h2(style=base::paste0("color:", cor_titulo, "; font-size:", tam_fonte_titulo), 
...
# # ==========================================================
# # --- FUNÇÃO: TABELA COM FILTROS PERSONALIZADOS (ALEX) ---
# # ==========================================================
# 
# tabela_interativa_detalhes <- function(dados, 
#                                        dados_detalhes,       
...
#   dados_processados <- dados
#   if (!base::is.null(colunas_selecionadas)) {
#     dados_processados <- dados_processados %>% dplyr::select(tidyselect::all_of(colunas_selecionadas))
#   }
#   
#   if (!base::is.null(coluna_ordenacao)) {
#     if (ordem_decrescente) {
#       dados_processados <- dados_processados %>% dplyr::arrange(dplyr::desc(.data[[coluna_ordenacao]]))
#     } else {
#       dados_processados <- dados_processados %>% dplyr::arrange(.data[[coluna_ordenacao]])
#     }
#   }
# 
#   dados_processados <- base::cbind(' ' = '&oplus;', dados_processados)
...
#     options = base::list(
#       dom = 'Bft p',
#       buttons = base::list('copy', 'excel', base::list(extend = 'pdfHtml5', title = titulo_final, orientation = 'landscape')),
#       pageLength = n_pages,
#       searching = pesquisa_geral,
#       ordering = FALSE,
#       initComplete = DT::JS(
...
#                      # Estilo Geral
#                      "$(this.api().table().container()).css({'font-size': '", tam_fonte_corpo, "', 'font-family': 'Arial, sans-serif'});",
#                      "$(this.api().table().header()).find('tr:first-child th').css({'border-top': '3px solid #000000', 'border-bottom': '3px solid #000000', 'padding': '12px', 'color': '", cor_primaria, "'});",
#                      
#                      # --- AJUSTE: REMOVE FILTRO DA PRIMEIRA COLUNA E PERSONALIZA OS DEMAIS ---
#                      "this.api().columns().every(function(index) {",
#                      "   var column = this;",
#                      "   if (index === 0) {", 
#                      "      $(column.filter()).empty();", # Mata o filtro da coluna do botão [+]
#                      "   } else {",
#                      "      var title = $(column.header()).text();",
#                      "      if (title.trim() !== '') {",
#                      "         $('input', column.filter()).attr('placeholder', 'Busca ' + title.trim());",
...
#                      "  if (row.child.isShown()) {",
#                      "    row.child.hide(); tr.removeClass('shown'); $(this).html('&oplus;');",
#                      "    tr.css({'background-color': '', 'font-weight': 'normal'});",
#                      "  } else {",
#                      "    tr.addClass('shown'); $(this).html('&ominus;');",
#                      "    tr.css({'background-color': '#f8f9fa', 'font-weight': 'bold'});",
#                      "    var rowData = row.data(); var chave = rowData[", if(mostrar_indice) "2" else "1", "];",
#                      "    var sub_dados = detalhes_data.filter(d => d['", coluna_chave, "'] == chave);",
#                      "    var html_sub = '<table class=\"subtable_alex\" style=\"width:97%; margin-left:3%; border-collapse: collapse;\"><thead><tr style=\"background:#ffffff; color:#03045e; font-weight:bold;\">';",
#                      "    var colKeys = Object.keys(sub_dados[0]);",
#                      "    colKeys.forEach((key, index) => {",
#                      "       var label = (customNomes && customNomes[index]) ? customNomes[index] : key;",
#                      "       html_sub += '<th style=\"padding:6px; border-bottom:1px solid #ddd; text-align:left;\">' + label + '</th>';",
#                      "    });",
#                      "    html_sub += '</tr></thead><tbody>';",
#                      "    sub_dados.forEach(r => {",
#                      "       html_sub += '<tr>';",
#                      "       colKeys.forEach(k => { html_sub += '<td style=\"padding:6px; background-color: transparent !important; font-weight: normal !important;\">' + r[k] + '</td>'; });",
#                      "       html_sub += '</tr>';",
#                      "    });",
#                      "    html_sub += '</tbody></table>';",
#                      "    row.child(html_sub).show();",
...
#   )
#   
#   return(htmltools::tagList(htmltools::tags$h2(style=base::paste0("color:", cor_titulo), htmltools::tags$b(titulo_final)), tabela))
# }
...
# ==========================================================
# --- 1. VETORES DE CONFIGURAÇÃO EXTERNOS (CURADORIA) ---
# ==========================================================

# --- TABELA PRINCIPAL (PAI) ---
# Mapeamento de nomes amigáveis para as colunas da base principal
...
# Definição de larguras da tabela principal (A primeira posição é o botão [+])
minhas_larguras <- base::c("30px", "150px", "100px", "150px", "80px", "140px", "100px")


# --- SUB-TABELA DE DETALHES (FILHA) ---
# Vetor com os nomes que aparecerão no cabeçalho quando o Alex clicar no [+]
...
# Definição de larguras para a sub-tabela (Garante que a descrição tenha espaço)
minhas_larguras_detalhes <- base::c("100px", "60px", "400px", "200px", "50px", "100px", "100px")


# ==========================================================
# --- 2. EXECUÇÃO DA TABELA COM USO DE TODOS OS PARÂMETROS ---
# ==========================================================

tabela_interativa_detalhes(
  # --- DADOS E VÍNCULO ---
  dados                  = bd_resumo_processos,  # Base de nível 1 (Resumo)
  dados_detalhes         = bd_detalhes_itens,    # Base de nível 2 (722 itens)
  coluna_chave           = "processo",           # Chave que liga as duas bases
  
  # --- TÍTULO E ORDENAÇÃO ---
  titulo_tabela          = "Gestão de Processos e Itens", # Título dinâmico do relatório
  coluna_ordenacao       = "processo",           # Variável alvo para a ordem inicial
  ordem_decrescente      = TRUE,                 # TRUE para Maior -> Menor (Z-A)
  
  # --- CUSTOMIZAÇÃO DOS DETALHES (USANDO OS VETORES EXTERNOS) ---
  colunas_detalhes_nomes = meus_nomes_detalhes,     # Aplica os nomes amigáveis nos itens
  largura_detalhes_vetor = minhas_larguras_detalhes, # Aplica as larguras fixas nos itens
  
  # --- DIMENSÕES DE FONTE (ESTILO ALEX) ---
  tam_fonte_corpo        = "11.3px",             # Fonte compacta para os dados das células
  tam_fonte_header       = "12.2px",             # Fonte para os cabeçalhos das colunas
  tam_fonte_titulo       = "14.3px",             # Fonte para o título da tabela (H2)
  
  # --- CORES E IDENTIDADE VISUAL ---
  cor_titulo             = "#03045e",            # Azul Marinho oficial para o título
  cor_primaria           = "#000000",            # Preto para cabeçalhos e destaque de seleção
  cor_texto_corpo        = "#000000",            # Preto para o texto padrão das linhas
  
  # --- CONFIGURAÇÕES DE INTERFACE ---
  mostrar_filtro         = TRUE,                 # Ativa busca por coluna (limpando a col do [+])
  pesquisa_geral         = TRUE,                 # Ativa a barra de busca "Search" global
  n_pages                = 10,                   # Registros visíveis por página
  mostrar_indice         = FALSE,                # FALSE para priorizar a coluna do botão [+]
  
  # --- MAPEAMENTO E LARGURAS DA TABELA PAI ---
  largura_cols_vetor     = minhas_larguras,      # Vetor de larguras da tabela principal
  colunas_selecionadas   = minhas_colunas        # Vetor de mapeamento amigável (Pai)
)
