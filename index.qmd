...
# ==========================================================
# --- FUNÇÃO: TABELA INTERATIVA (VERSÃO COMPATÍVEL CROSSTALK) ---
# ==========================================================
tabela_interativa <- function(dados, titulo_tabela, tam_fonte_corpo = "13px", tam_fonte_titulo = "20px", 
                              tam_fonte_header = "14px", cor_titulo = "#03045e", cor_primaria = "#03045e", 
                              cor_texto_corpo = "#333333", mostrar_filtro = TRUE, pesquisa_geral = FALSE, 
                              largura_cols_vetor = NULL, colunas_selecionadas = NULL) {

  # Tratamento para SharedData (Crosstalk)
  is_shared <- inherits(dados, "SharedData")
  df_interno <- if(is_shared) dados$origData() else dados

  if (!base::exists("contador_tabelas_alex")) { base::assign("contador_tabelas_alex", 0, envir = .GlobalEnv) }

  # Seleção de colunas antes da renderização
  if (!base::is.null(colunas_selecionadas)) {
    df_interno <- df_interno %>% dplyr::select(tidyselect::all_of(colunas_selecionadas))
    if(is_shared) {
      dados <- crosstalk::SharedData$new(df_interno, group = dados$groupName())
    } else { dados <- df_interno }
  }

  n_colunas_dados <- base::ncol(df_interno)
  lista_columnDefs <- base::list(base::list(width = '30px', targets = 0))
  if (!base::is.null(largura_cols_vetor)) {
    for (i in base::seq_along(largura_cols_vetor)) {
      if (i <= (n_colunas_dados + 1)) {
         lista_columnDefs[[base::length(lista_columnDefs) + 1]] <- base::list(width = largura_cols_vetor[i], targets = i - 1)
      }
    }
  }
  ...
  html_titulo <- htmltools::tags$h2(
    style = base::paste0("color:", cor_titulo, "; font-size:", tam_fonte_titulo, "; font-weight:bold; margin-bottom:10px;"), 
    base::paste0("Tabela ", contador_tabelas_alex, ": ", titulo_tabela)
  )

  tabela <- DT::datatable(
    dados, 
    rownames = TRUE, 
    filter = if(mostrar_filtro) base::list(position = "top") else "none",
    width = "100%", 
    class = "hover row-border", 
    options = base::list(
      dom = 'ft p', 
      ...
      scrollX = TRUE,
      columnDefs = lista_columnDefs,
      initComplete = DT::JS(base::paste0("function(settings, json) {",
                                         "$(this.api().table().container()).css({'font-size': '", tam_fonte_corpo, "', 'font-family': 'Inter, sans-serif'});",
                                         "$(this.api().table().header()).css({'background-color': '#ffffff', 'color': '", cor_primaria, "'});",
                                         "$(this.api().table().header()).find('tr:first-child th').css({'border-top': '3px solid #000000', 'border-bottom': '3px solid #000000', 'padding': '12px'});",
                                         "$(this.api().table().node()).css({'border-bottom': '3px solid #000000'});",
                                         if(pesquisa_geral) "" else "$('.dataTables_filter').hide();", "}"))
    ),
    language = base::list(paginate = base::list("previous" = "Anterior", "next" = "Próximo"), search = "Pesquisa:")
  )
...
# --- DICIONÁRIO E ORDEM ---
meses_ordem_limpo <- c("Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro")
tradutor_meses <- c("January"="Janeiro", "February"="Fevereiro", "March"="Março", "April"="Abril", "May"="Maio", "June"="Junho", "July"="Julho", "August"="Agosto", "September"="Setembro", "October"="Outubro", "November"="Novembro", "December"="Dezembro")

# --- REESTRUTURAÇÃO ---
dados_base_limpa <- bd_principal %>%
  dplyr::distinct(processo, .keep_all = TRUE) %>%
  # Selecionamos todas as colunas que você pediu para a tabela
  dplyr::select(
    processo, mes_entrada, mes_saida, modalidade_processo, status_item, 
    agente, qtd_itens_processo, natureza_despesa, valor_total, pct_empenho
  ) %>%
  ...
  dplyr::mutate(
    Mes_Nome_Limpo = dplyr::recode(Mes_Nome, !!!tradutor_meses),
    Mes_Bold = factor(paste0("<b>", Mes_Nome_Limpo, "</b>"), levels = paste0("<b>", meses_ordem_limpo, "</b>"))
  )
...
minhas_colunas <- c(
  ...
  "Itens"            = "qtd_itens_processo", 
  "Natureza"         = "natureza_despesa", 
  "Valor Total (R$)" = "valor_total", 
  "Empenho (%)"      = "pct_empenho"
)

# 6 colunas + 1 numeração = 7 larguras
minhas_larguras <- c("30px", "150px", "120px", "60px", "130px", "100px", "80px")

# --- 2. CRIAÇÃO DO OBJETO COMPARTILHADO ---
sd <- crosstalk::SharedData$new(dados_base_limpa)

# --- 3. EXIBIÇÃO FINAL ---
htmltools::tagList(
  # Filtros que comandam ambos
  crosstalk::bscols(widths = c(6, 6),
         crosstalk::filter_select("f_mod", "Selecione a Modalidade", sd, ~modalidade_processo),
         crosstalk::filter_select("f_stat", "Filtrar por Status", sd, ~status_item)),
  br(),
  
  # Gráfico Dinâmico (Sincronizado)
  plotly::plot_ly(sd, x = ~Mes_Bold) %>%
    plotly::add_histogram(y = ~ifelse(Tipo == "Entradas", 1, 0), name = "Entradas", 
                          marker = list(color = "#001d3d"), histfunc = "sum") %>%
    plotly::add_histogram(y = ~ifelse(Tipo == "Saídas", 1, 0), name = "Saídas", 
                          marker = list(color = "#48cae4"), histfunc = "sum") %>%
    plotly::layout(barmode = "group", xaxis = list(title = ""), yaxis = list(title = "Qtd Processos"),
                   title = list(text = "<b>Fluxo Operacional de Processos</b>", x = 0),
                   font = list(family = "Inter, sans-serif")),
  br(), br(),
  
  # Tabela Interativa (Sincronizada)
  tabela_interativa(
    dados = sd, 
