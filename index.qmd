...
# ==========================================================
# --- FUNÇÃO: TABELA INTERATIVA PREMIUM (CORREÇÃO DE FOOTER) ---
# ==========================================================
...
tabela_interativa_detalhes_premium <- function(dados, 
  ...
  dados_processados <- dados
  if (!base::is.null(colunas_selecionadas)) {
    # Filtra apenas as colunas que o Alex selecionou
    dados_processados <- dados %>% dplyr::select(tidyselect::all_of(colunas_selecionadas))
  }

  # --- 2. ADIÇÃO DA COLUNA DE CONTROLE (BOTÃO +) ---
  # Insere a coluna vazia para o ícone de expansão na posição zero
  dados_processados <- base::cbind(' ' = '&oplus;', dados_processados)
  ...
  # Título formatado com as variáveis de cor e tamanho do Alex
  html_titulo <- htmltools::tags$h2(
    style = base::paste0("color: ", cor_titulo, "; font-size: ", tam_fonte_titulo, 
                         "; font-weight: bold; text-align: left; margin-bottom: 10px;"),
    htmltools::tags$b(titulo_final)
  ...
  # --- 4. PREPARAÇÃO DOS DADOS PARA JAVASCRIPT ---
  json_detalhes <- jsonlite::toJSON(dados_detalhes)
  ...
  tabela <- DT::datatable(
    base::data.frame(dados_processados, check.names = FALSE),
    escape = FALSE, # Permite ícones HTML
    rownames = mostrar_indice,
    # Lógica do filtro superior
    filter = if(mostrar_filtro) base::list(position = "top", clear = TRUE, plain = TRUE) else "none",
    width = "100%", 
    class = "hover row-border", 
    options = base::list(
      ...
      autoWidth = FALSE,    
      scrollX = TRUE,
      # Força a coluna zero (ícone) a ser pequena e não ordenável
      columnDefs = base::list(
        base::list(visible = TRUE, targets = 0, className = 'details-control', width = '20px', orderable = FALSE)
      ),
      
      # CALLBACK PARA TOTAIS NO RODAPÉ (Lógica JavaScript para somar Valor Total)
      footerCallback = DT::JS(
        ...
        "  $(api.column('.valor_total_proc').footer()).html('<b>Total: R$ ' + total.toLocaleString('pt-BR', {minimumFractionDigits: 2}) + '</b>');",
        "  $(api.column(1).footer()).html('<b>Resumo da Página</b>');",
        "}"
      ),

      initComplete = DT::JS(
        base::paste0("function(settings, json) {",
                     # Estilos visuais originais (Alex)
                     "$(this.api().table().container()).css({'font-size': '", tam_fonte_corpo, "', 'font-family': 'Arial, sans-serif', 'color': '", cor_texto_corpo, "'});",
                     "$(this.api().table().header()).css({'font-size': '", tam_fonte_header, "', 'background-color': '#ffffff', 'color': '", cor_primaria, "', 'white-space': 'nowrap'});",
                     "$(this.api().table().header()).find('tr:first-child th').css({'border-top': '3px solid #000000', 'border-bottom': '3px solid #000000', 'padding': '12px'});",
                     "$(this.api().table().node()).css({'border-bottom': '3px solid #000000'});",
                     
                     # Estilo Pesquisa Geral (Alinhada à esquerda como você gosta)
                     "var searchDiv = $('.dataTables_filter'); searchDiv.css({'text-align': 'left', 'margin-bottom': '15px', 'float': 'none'});",
                     "searchDiv.find('label').css({'font-weight': 'bold', 'color': '#000000'});",
                     if(pesquisa_geral) "" else "$('.dataTables_filter').hide();",
                     
                     # 1. PÍLULAS DE STATUS (Cores semafóricas)
                     ...
                     # 2. LÓGICA DE CHILD ROWS (Expansão ao clicar no +)
                     "var table = this.api();",
                     "var detalhes_data = ", json_detalhes, ";",
                     "var customNomes = ", nomes_detalhes_js, ";",
                     "var customLarguras = ", larguras_detalhes_js, ";",
                     ...
                     # 3. HOVER CUSTOMIZADO (Alex)
                     "var tableContainer = $(this.api().table().container());",
                     "tableContainer.find('tbody').on('mouseenter', 'tr', function() { $(this).css({'color': '", cor_primaria, "', 'font-weight': '600', 'transition': '0.2s'}); }).on('mouseleave', 'tr', function() { $(this).css({'color': '", cor_texto_corpo, "', 'font-weight': 'normal'}); });",
                     "}")
      ),
      # Tradução dos controles de paginação
      language = base::list(
        paginate = base::list("previous" = "Anterior", "next" = "Próximo"),
        emptyTable = "Sem dados", zeroRecords = "Nenhum registro encontrado", search = "Pesquisa Geral:"
      )
  ...
  # Retorno consolidado com título e tabela
  htmltools::tagList(html_titulo, tabela)
}
# ==============================================================================
