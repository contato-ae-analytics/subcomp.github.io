...
tabela_interativa <- function(dados, 
                              ...
                              n_pages = 10,
                              mostrar_indice = TRUE) {      # NOVO: Argumento para mostrar/ocultar índice
  ...
  html_titulo <- htmltools::tags$h2(
    ...
    htmltools::tags$b(titulo_final) # <--- ALTERAÇÃO: Título em negrito
  )

  # --- 5. RENDERIZAÇÃO DA TABELA (DT) ---
  tabela <- DT::datatable(
    base::data.frame(dados_processados, check.names = FALSE),
    rownames = mostrar_indice,                      # <--- APLICAÇÃO: mostrar ou não o índice
...
# ==========================================================
# --- FUNÇÃO: TABELA INTERATIVA COM DETALHES (CHILD ROWS) ---
# ==========================================================

tabela_interativa_detalhes <- function(dados, 
                                       dados_detalhes,       # NOVO: DF com os subitens
                                       coluna_chave,         # NOVO: Nome da coluna que liga as duas tabelas
                                       titulo_tabela, 
                                       tam_fonte_corpo = "13px",   
                                       tam_fonte_titulo = "20px",  
                                       tam_fonte_header = "14px",  
  ...
  # --- 1. PREPARAÇÃO DOS DADOS PRINCIPAIS ---
  dados_processados <- dados
  if (!base::is.null(colunas_selecionadas)) {
    dados_processados <- dados %>% dplyr::select(tidyselect::all_of(colunas_selecionadas))
  }

  # --- 2. ADIÇÃO DA COLUNA DE CONTROLE (BOTÃO +) ---
  # Criamos a coluna vazia no início para o botão de expansão
  ...
  # --- 5. LÓGICA DE DETALHES (JSON PARA JAVASCRIPT) ---
  # Transformamos os detalhes em JSON para o navegador processar
  json_detalhes <- jsonlite::toJSON(dados_detalhes)

  # --- 6. RENDERIZAÇÃO DA TABELA ---
  tabela <- DT::datatable(
    base::data.frame(dados_processados, check.names = FALSE),
    escape = FALSE, # Permite renderizar o ícone HTML &oplus;
    ...
    options = base::list(
      ...
      autoWidth = FALSE,    
      scrollX = TRUE,
      # Define que a primeira coluna (ícone) é pequena e centralizada
      columnDefs = base::list(
        base::list(visible = TRUE, targets = 0, className = 'details-control', width = '20px', orderable = FALSE)
      ),
      
      # JAVASCRIPT: LÓGICA DE CLIQUE E EXIBIÇÃO
      initComplete = DT::JS(
        base::paste0("function(settings, json) {",
                     # Estilo do corpo
                     "$(this.api().table().container()).css({'font-size': '", tam_fonte_corpo, "', 'font-family': 'Arial, sans-serif', 'color': '", cor_texto_corpo, "'});",
                     # Cabeçalho: Estilo Branco/Azul com Moldura Preta do Alex
                     "$(this.api().table().header()).css({'font-size': '", tam_fonte_header, "', 'background-color': '#ffffff', 'color': '", cor_primaria, "', 'white-space': 'nowrap', 'background-image': 'none !important'});",
                     "$(this.api().table().header()).find('tr:first-child th').css({'border-top': '3px solid #000000', 'border-bottom': '3px solid #000000', 'padding': '12px'});",
                     "$(this.api().table().node()).css({'border-bottom': '3px solid #000000'});",
                     
                     # Estilo Pesquisa Geral
                     "var searchDiv = $('.dataTables_filter'); searchDiv.css({'text-align': 'left', 'margin-bottom': '15px', 'float': 'none'});",
                     "searchDiv.find('label').css({'font-weight': 'bold', 'color': '#000000'});",
                     if(pesquisa_geral) "" else "$('.dataTables_filter').hide();",
                     
                     # Lógica de expansão ao clicar
                     "var table = this.api();",
                     "var detalhes_data = ", json_detalhes, ";", # Passa os dados extras para o JS
                     ...
                     "    var chave = rowData[", if(mostrar_indice) "2" else "1", "];", # Ajusta índice da chave conforme rownames
                     "    var sub_dados = detalhes_data.filter(d => d['", coluna_chave, "'] == chave);",
                     
                     # Cria a tabela interna de subitens
                     "    var html_sub = '<table style=\"width:95%; margin-left:5%; border:1px solid #ddd;\"><thead><tr style=\"background:#f9f9f9\">';",
                     "    for (var key in sub_dados[0]) { html_sub += '<th>' + key + '</th>'; }",
                     "    html_sub += '</tr></thead><tbody>';",
                     "    sub_dados.forEach(r => { html_sub += '<tr>'; for (var k in r) { html_sub += '<td>' + r[k] + '</td>'; } html_sub += '</tr>'; });",
...
tabela_interativa_detalhes_2 <- function(dados, 
                                       dados_detalhes,       # DF com os subitens
                                       coluna_chave,         # Nome da coluna que liga as duas tabelas
                                       titulo_tabela, 
                                       colunas_detalhes_nomes = NULL, # NOVO: Nomes amigáveis para os detalhes
                                       largura_detalhes_vetor = NULL, # NOVO: Tamanhos das colunas dos detalhes
                                       tam_fonte_corpo = "13px",   
                                       tam_fonte_titulo = "20px",  
                                       tam_fonte_header = "14px",  
                                       cor_titulo = "#03045e",        
  ...
  # Preparação dos nomes das colunas de detalhes para o JS
  nomes_detalhes_js <- if(!base::is.null(colunas_detalhes_nomes)) {
    jsonlite::toJSON(colunas_detalhes_nomes)
  } else {
    "null"
  }
  
  # Preparação das larguras das colunas de detalhes para o JS
  larguras_detalhes_js <- if(!base::is.null(largura_detalhes_vetor)) {
    jsonlite::toJSON(largura_detalhes_vetor)
  } else {
    "null"
  }
  ...
  tabela <- DT::datatable(
    base::data.frame(dados_processados, check.names = FALSE),
    escape = FALSE, # Permite renderizar o ícone HTML
    rownames = mostrar_indice,
    filter = if(mostrar_filtro) base::list(position = "top", clear = TRUE, plain = TRUE) else "none",
    width = "100%", 
    class = "hover row-border", 
    options = base::list(
      ...
      initComplete = DT::JS(
        base::paste0("function(settings, json) {",
                     # Estilo do corpo e cabeçalho (Moldura Preta Alex)
                     "$(this.api().table().container()).css({'font-size': '", tam_fonte_corpo, "', 'font-family': 'Arial, sans-serif', 'color': '", cor_texto_corpo, "'});",
                     "$(this.api().table().header()).css({'font-size': '", tam_fonte_header, "', 'background-color': '#ffffff', 'color': '", cor_primaria, "', 'white-space': 'nowrap', 'background-image': 'none !important'});",
                     "$(this.api().table().header()).find('tr:first-child th').css({'border-top': '3px solid #000000', 'border-bottom': '3px solid #000000', 'padding': '12px'});",
                     "$(this.api().table().node()).css({'border-bottom': '3px solid #000000'});",
                     ...
                     # Lógica Child Rows com Customização de Colunas e Nomes
                     "var table = this.api();",
                     "var detalhes_data = ", json_detalhes, ";",
                     "var customNomes = ", nomes_detalhes_js, ";",
                     "var customLarguras = ", larguras_detalhes_js, ";",
                     ...
                     "  } else {",
                     "    var rowData = row.data();",
                     "    var chave = rowData[", if(mostrar_indice) "2" else "1", "];",
                     "    var sub_dados = detalhes_data.filter(d => d['", coluna_chave, "'] == chave);",
                     
                     # Montagem da Sub-tabela com cabeçalhos e larguras dinâmicas
                     "    var html_sub = '<table style=\"width:95%; margin-left:5%; border-collapse: collapse; border: 1px solid #ddd;\"><thead><tr style=\"background:#f3f3f3; color:#03045e; font-weight:bold;\">';",
                     ...
                     "       html_sub += '<th style=\"padding:8px; border-bottom:2px solid #000; text-align:left; '+width+'\">' + label + '</th>';",
                     "    });",
                     "    html_sub += '</tr></thead><tbody>';",
                     "    sub_dados.forEach(r => {",
                     "       html_sub += '<tr style=\"border-bottom:1px solid #eee;\">';",
                     "       colKeys.forEach(k => { html_sub += '<td style=\"padding:8px;\">' + r[k] + '</td>'; });",
                     "       html_sub += '</tr>';",
                     "    });",
                     "    html_sub += '</tbody></table>';",
...
# ==========================================================
# --- FUNÇÃO: TABELA INTERATIVA PREMIUM (COM 3 MELHORIAS) ---
# ==========================================================

tabela_interativa_detalhes_premium <- function(dados, 
                                              dados_detalhes,       
                                              coluna_chave,         
                                              titulo_tabela, 
  ...
  # Captura o índice da coluna de Valor para o Heatmap e o Totalizador
  idx_valor <- base::which(base::names(dados_processados) == "valor_total_proc")
  # Captura o índice da coluna de Status
  idx_status <- base::which(base::names(dados_processados) == "status")
  ...
  tabela <- DT::datatable(
    base::data.frame(dados_processados, check.names = FALSE),
    escape = FALSE,
    container = htmltools::withTags(base::table(
      class = "display",
      htmltools::tags$thead(DT::tableHeader(base::names(dados_processados))),
      htmltools::tags$tfoot(base::tr(base::lapply(base::names(dados_processados), function(x) htmltools::tags$th(""))))
    )),
    rownames = mostrar_indice,
    filter = if(mostrar_filtro) base::list(position = "top", clear = TRUE, plain = TRUE) else "none",
    width = "100%", 
    ...
    options = base::list(
      ...
      lengthChange = FALSE,
      info = FALSE,
      ordering = TRUE, # Ativado para o Heatmap fazer sentido      
      autoWidth = FALSE,    
      scrollX = TRUE,
      columnDefs = base::list(
        base::list(visible = TRUE, targets = 0, className = 'details-control', width = '20px', orderable = FALSE)
      ...
      # CALLBACK PARA TOTAIS NO RODAPÉ (SUGESTÃO 3)
      footerCallback = DT::JS(
        "function(row, data, start, end, display) {",
        "  var api = this.api();",
        "  var intVal = function(i) { return typeof i === 'string' ? i.replace(/[\\R$,.]/g, '')*1/100 : typeof i === 'number' ? i : 0; };",
        "  var total = api.column('.valor_total_proc', {page: 'current'}).data().reduce(function(a, b) { return intVal(a) + intVal(b); }, 0);",
        "  $(api.column('.valor_total_proc').footer()).html('Total: R$ ' + total.toLocaleString('pt-BR', {minimumFractionDigits: 2}));",
        "  $(api.column(1).footer()).html('Resumo da Página');",
        "}"
      ),

      initComplete = DT::JS(
        base::paste0("function(settings, json) {",
                     # Estilos originais Alex
                     "$(this.api().table().container()).css({'font-size': '", tam_fonte_corpo, "', 'font-family': 'Arial, sans-serif'});",
                     "$(this.api().table().header()).css({'background-color': '#ffffff', 'color': '", cor_primaria, "'});",
                     "$(this.api().table().header()).find('tr:first-child th').css({'border-top': '3px solid #000000', 'border-bottom': '3px solid #000000', 'padding': '12px'});",
                     "$(this.api().table().node()).css({'border-bottom': '3px solid #000000'});",
                     
                     # 1. PÍLULAS DE STATUS (SUGESTÃO 1)
                     "$(this.api().table().body()).find('td').each(function() {",
                     "  var txt = $(this).text();",
                     "  if(txt === 'EMPENHADO') $(this).html('<span style=\"background:#d1e7dd; color:#0f5132; padding:3px 8px; border-radius:12px; font-weight:bold; font-size:10px;\">EMPENHADO</span>');",
                     "  if(txt === 'PENDENTE') $(this).html('<span style=\"background:#fff3cd; color:#664d03; padding:3px 8px; border-radius:12px; font-weight:bold; font-size:10px;\">PENDENTE</span>');",
                     "  if(txt === 'CANCELADO') $(this).html('<span style=\"background:#f8d7da; color:#842029; padding:3px 8px; border-radius:12px; font-weight:bold; font-size:10px;\">CANCELADO</span>');",
                     "});",

                     # 2. HEATMAP/BARRA DE PROGRESSO (SUGESTÃO 2)
                     # Esta parte exige que os dados financeiros sejam processados para identificar o maior valor
                     
                     # Lógica Child Rows e Pesquisa (mantidas)
                     "var table = this.api(); var detalhes_data = ", json_detalhes, ";",
                     "$(this.api().table().body()).on('click', 'td.details-control', function() {",
                     "  var tr = $(this).closest('tr'); var row = table.row(tr);",
                     "  if (row.child.isShown()) { row.child.hide(); tr.removeClass('shown'); $(this).html('&oplus;'); }",
                     "  else {",
                     "    var rowData = row.data(); var chave = rowData[", if(mostrar_indice) "2" else "1", "];",
                     "    var sub_dados = detalhes_data.filter(d => d['", coluna_chave, "'] == chave);",
                     "    var html_sub = '<table style=\"width:95%; margin-left:5%; border-collapse:collapse; border:1px solid #ddd;\"><thead><tr style=\"background:#f3f3f3; color:#03045e;\">';",
                     "    Object.keys(sub_dados[0]).forEach((key, i) => { html_sub += '<th style=\"padding:8px; border-bottom:2px solid #000;\">' + (", nomes_detalhes_js, " && ", nomes_detalhes_js, "[i] ? ", nomes_detalhes_js, "[i] : key) + '</th>'; });",
                     "    html_sub += '</tr></thead><tbody>';",
                     "    sub_dados.forEach(r => { html_sub += '<tr>'; Object.values(r).forEach(v => { html_sub += '<td style=\"padding:8px; border-bottom:1px solid #eee;\">' + v + '</td>'; }); html_sub += '</tr>'; });",
                     "    html_sub += '</tbody></table>';",
                     "    row.child(html_sub).show(); tr.addClass('shown'); $(this).html('&ominus;');",
                     "  }",
                     "});",
...
# # titulo_cor        <- "#001d3d"
# titulo_cor        <- "#03045e"
# titulo_tamanho    <- 14.5         # Ajustado para destaque (era 10)
# sub_cor           <- "#000000"
# sub_tamanho       <- 13         
# 
...
plotly::plot_ly() %>%
  ...
  plotly::layout(
    ...
    margin = base::list(t = 150, b = 170, l = 30, r = 30),
    plot_bgcolor = "white", paper_bgcolor = "white"
  ) %>%
...
# ==========================================================
# --- 3. CONFIGURAÇÃO VISUAL COMPACTA (ESTILO ALEX) ---
# ==========================================================

# Definição das larguras das colunas (Posição 0 é o botão de expansão)
minhas_larguras <- base::c(
  "30px",   # Coluna do botão [+]
  "150px",  # Processo
  "100px",  # Modalidade
  "150px",  # Agente
  "80px",   # Qtd Itens
...
# Mapeamento de colunas para exibição amigável
...
tabela_interativa_detalhes(
  ...
  mostrar_indice = FALSE, # Oculta o índice para usar o botão [+]
  n_pages = 10,
  pesquisa_geral = TRUE # Ativa a busca no topo
)


# Alex, defina os parâmetros dos detalhes assim:
meus_nomes_detalhes <- c("Código Item", "Nome do Material", "Fornecedor Ativo", "Quantidade", "V. Unit", "V. Total")
minhas_larguras_detalhes <- c("80px", "350px", "200px", "60px", "100px", "100px")

tabela_interativa_detalhes_2(
  dados = bd_resumo_processos,
  dados_detalhes = bd_detalhes_itens,
  coluna_chave = "processo",
  titulo_tabela = "Gestão de Itens por Processo",
  
  # AQUI AS SUAS NOVAS ALTERAÇÕES:
...
# ==========================================================
# --- EXEMPLO DE APLICAÇÃO PARA O ALEX ---
# ==========================================================

# 1. Definimos as colunas da tabela principal (Processos)
minhas_colunas_principais <- base::c(
  "Processo"      = "processo",
  "Modalidade"    = "modalidade",
  "Agente"        = "agente",
  "Itens"         = "total_itens",
  "Valor Total"   = "valor_total_proc", # Importante: manter este nome para o totalizador
  "Status"        = "status"            # Importante: manter este nome para as pílulas
)

# 2. Definimos os nomes amigáveis para os detalhes (Itens)
...
# 3. Definimos as larguras das colunas dos detalhes
minhas_larguras_detalhes <- base::c(
  "80px",   # ID
  "350px",  # Descrição
  "200px",  # Fornecedor
  "70px",   # Qtd
...
# 4. Chamada da Função Premium
tabela_interativa_detalhes_premium(
  dados = bd_resumo_processos,      # Sua tabela agrupada (1 linha por processo)
  dados_detalhes = bd_detalhes_itens, # Sua tabela com os 722 itens
  coluna_chave = "processo",         # A ligação entre elas
  titulo_tabela = "Acompanhamento de Processos de Compra",
  
  # --- AS SUAS VARIÁVEIS DE SELEÇÃO E ESTILO (MANTIDAS) ---
  colunas_selecionadas = minhas_colunas_principais,
  colunas_detalhes_nomes = meus_nomes_detalhes,
  largura_detalhes_vetor = minhas_larguras_detalhes,
  
  tam_fonte_corpo = "11.3px",   # Seu padrão compacto
  tam_fonte_header = "12.2px",
  tam_fonte_titulo = "14.3px",
  cor_primaria = "#000000",     # Preto conforme seu estilo
  cor_titulo = "#03045e",       # Azul escuro para o título
  
  mostrar_indice = FALSE,       # Sem números de linha
  pesquisa_geral = TRUE,        # Barra de busca ativa
  n_pages = 10                  # Quantidade de processos por página
)
...
tam_fonte_corpo = "11.3px"     # Fonte dos dados: pequena para maximizar a visualização
tam_fonte_titulo = "14.3px"   # Título: discreto e elegante em azul oficial
tam_fonte_header = "12.2px"   # Cabeçalho: equilibrado entre as linhas pretas grossas
cor_titulo = "#03045e"        # NOVA: Cor exclusiva do Título
