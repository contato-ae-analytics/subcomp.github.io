...
# Base para o Gráfico (Resumo por Mês e Tipo)
dados_resumo <- bd_principal %>%
  dplyr::distinct(processo, .keep_all = TRUE) %>%
  dplyr::select(processo, mes_entrada, mes_saida, modalidade_processo) %>%
  tidyr::pivot_longer(cols = c(mes_entrada, mes_saida), 
                      names_to = "Tipo_Data", 
                      values_to = "Mes") %>%
  dplyr::mutate(Tipo = ifelse(Tipo_Data == "mes_entrada", "Entradas", "Saídas")) %>%
  dplyr::filter(!is.na(Mes)) %>%
  # Garantir Primeira Letra Maiúscula
  dplyr::mutate(Mes = stringr::str_to_title(as.character(Mes)))

# Cálculo de Porcentagem por Modalidade para o Tooltip Dinâmico
breakdown_modalidade <- dados_resumo %>%
  dplyr::group_by(Mes, Tipo, modalidade_processo) %>%
  dplyr::summarise(n = n(), .groups = "drop") %>%
  dplyr::group_by(Mes, Tipo) %>%
  dplyr::mutate(pct = (n / sum(n)) * 100) %>%
  dplyr::summarise(
    detalhe_modalidade = paste0(modalidade_processo, ": ", round(pct, 1), "%", collapse = "<br>"),
    total_mes = sum(n),
    .groups = "drop"
  )

# Unindo tudo
dados_finais_alex <- dados_resumo %>%
  dplyr::group_by(Mes, Tipo) %>%
  dplyr::summarise(Quantidade = n(), .groups = "drop") %>%
  dplyr::left_join(breakdown_modalidade, by = c("Mes", "Tipo"))





# --- 1. REGISTRO DE FONTES DO SEU CABEÇALHO ---
sysfonts::font_add_google("Inter", "inter")
sysfonts::font_add_google("Fira Code", "fira_code")
showtext::showtext_auto()

# --- 2. PREPARAÇÃO DOS DADOS (PROCESSOS ÚNICOS) ---
dados_fluxo_rosa <- bd_principal %>%
  dplyr::select(processo, mes_entrada, mes_saida) %>%
  dplyr::distinct(processo, .keep_all = TRUE) %>%
  dplyr::group_by(Mes = mes_entrada) %>%
  dplyr::summarise(Entradas = dplyr::n()) %>%
  dplyr::left_join(
    bd_principal %>%
      dplyr::distinct(processo, .keep_all = TRUE) %>%
      dplyr::group_by(Mes = mes_saida) %>%
      dplyr::summarise(Saídas = dplyr::n()),
    by = "Mes"
  ) %>%
  tidyr::pivot_longer(cols = c("Entradas", "Saídas"), 
                      names_to = "Tipo", 
                      values_to = "Quantidade") %>%
  dplyr::filter(!base::is.na(Mes))
...
# --- 3. GRÁFICO COM A PALETA ROSA E FONTE INTER ---
cor_entrada_vinho <- "#a4133c" # O rosa escuro/vinho
cor_saida_rosa    <- "#ff4d6d" # O rosa vibrante

grafico_rosa_executivo <- ggplot2::ggplot(dados_fluxo_rosa, 
                                          ggplot2::aes(x = Mes, y = Quantidade, fill = Tipo)) +
  # Barras com espaçamento elegante
  ggplot2::geom_bar(stat = "identity", 
                    position = ggplot2::position_dodge(width = 0.8), 
                    width = 0.7) +
  
  
  # Rótulos de dados (10px) - Usando a fonte Inter
  ggplot2::geom_text(ggplot2::aes(label = Quantidade), 
                     position = ggplot2::position_dodge(width = 0.8), 
                     vjust = -0.6, 
                     size = 3.5, # Aprox 10px
                     fontface = "bold", 
  ...
  # Aplicação das cores solicitadas
  ggplot2::scale_fill_manual(values = c("Entradas" = cor_entrada_vinho, "Saídas" = cor_saida_rosa)) +
  
  # Títulos e Identidade (Usando Inter)
  ggplot2::labs(
    title = "FLUXO OPERACIONAL DE PROCESSOS | SUBCOMP",
    subtitle = "Comparativo mensal de entradas e saídas (Fontes: Inter & Fira Code)",
    x = NULL, y = "QUANTIDADE", fill = NULL
  ) +
  
  # Tema personalizado para combinar com o HTML do seu relatório
  ggplot2::theme_minimal(base_family = "inter") +
  ggplot2::theme(
    # Título (16px)
    plot.title = ggplot2::element_text(size = 16, face = "bold", color = "#a4133c"),
    # Subtítulo (11px)
    plot.subtitle = ggplot2::element_text(size = 11, color = "grey30", margin = margin(b=15)),
    
    # Eixos (10px e 11px)
    axis.text.x = ggplot2::element_text(size = 11, face = "bold", color = "black"),
    axis.text.y = ggplot2::element_text(size = 10, color = "grey50"),
    axis.title.y = ggplot2::element_text(size = 10, face = "bold", color = "#a4133c"),
    legend.position = "top",
    ...
    panel.grid.major.y = ggplot2::element_line(color = "#f8f9fa"),
    plot.background = ggplot2::element_rect(fill = "white", color = NA),
    plot.margin = margin(20, 20, 20, 20)
  )

# Renderizar
base::print(grafico_rosa_executivo)
