...
gerar_dashboard_setor <- function(bd_principal,
  ...
  df_modalidade <- df_proc_unico %>%
    ...
    dplyr::mutate(
      ...
      label_grafico = base::paste0("<b>", modalidade, "</b><br>", 
                                   n_processos, "<br>", 
      ...
      hover_txt = base::paste0("<span style='color:black; font-family:", fonte_padrao, "; font-size:", tam_hover_corpo, "px;'>",
                               ...
                               "📊 <b>Eficiência: ", p_res, "%</b> | ⏱ <b>Ciclo: ", base::round(tempo_medio_mod, 0), " dias</b><br>",
                               "📦 <b>Volume Total: ", n_processos, " processos</b><br>",
                               "<span style='color:#bdc3c7'>──────────────────────────</span><br>",
  ...
  # --- 3. MÉTRICAS DO CENTRO (ESTRUTURA SOLICITADA) ---
  total_p <- base::nrow(df_proc_unico)
  total_v <- base::sum(df_proc_unico$valor_processo, na.rm = TRUE)
  
  res_n <- base::sum(df_proc_unico$situacao_proc == "Resolvido")
  res_v <- base::sum(df_proc_unico$valor_empenhado, na.rm = TRUE)
  res_p <- base::sprintf("%.1f", (res_n / total_p) * 100)
  
  pen_n <- total_p - res_n
  pen_v <- total_v - res_v
  pen_p <- base::sprintf("%.1f", (pen_n / total_p) * 100)
  
  eficiencia_global <- base::sprintf("%.1f", (res_n / total_p) * 100)
  
  # Formatação dos valores financeiros
  f_v_total <- base::format(total_v, big.mark = ".", decimal.mark = ",")
  f_v_res   <- base::format(res_v, big.mark = ".", decimal.mark = ",")
  f_v_pen   <- base::format(pen_v, big.mark = ".", decimal.mark = ",")
  
  texto_centro <- base::paste0(
    "<span style='font-family:", fonte_padrao, ";'>",
    "📊 <b><span style='color:", cor_eficiencia, "; font-size:", tam_centro_cab, "px;'>EFICIÊNCIA: ", eficiencia_global, "%</span></b><br>",
    "<span style='color:#bdc3c7'>────────────────────────</span><br>",
    espacador_bloco,
    "<b><span style='font-size:", tam_centro_corpo, "px;'>TOTAL: ", total_p, " | R$ ", f_v_total, " | 100%</span></b><br>",
    espacador_frase,
    "<b><span style='color:", cor_empenhado, "; font-size:", tam_centro_corpo, "px;'>OK: ", res_n, " | R$ ", f_v_res, " | ", res_p, "%</span></b><br>",
    espacador_frase,
    "<b><span style='color:", cor_pendente, "; font-size:", tam_centro_corpo, "px;'>PEND: ", pen_n, " | R$ ", f_v_pen, " | ", pen_p, "%</span></b><br>",
    "</span>"
  )
  ...
  plotly::plot_ly(df_modalidade, labels = ~label_grafico, values = ~n_processos, type = 'pie', hole = 0.75,
    ...
    plotly::layout(
      margin = base::list(t = 50, b = 50, l = 50, r = 50), 
      title = base::list(text = "", font = base::list(family = fonte_padrao, size = tam_titulo_graf, color = cor_titulo)),
      font = base::list(family = fonte_padrao, size = tam_fatias),
      showlegend = FALSE,
      hoverlabel = base::list(bgcolor = "white", bordercolor = "black", align = "left", namelength = -1),
...
gerar_dashboard_setor(
  ...
  tam_hover_header = 9,        # Título da Modalidade no balão (Destaque)
  tam_hover_dest   = 9,        # Eficiência dentro do balão
  tam_hover_corpo  = 9,        # Informações detalhadas do balão
  
  # --- Rótulos e Cores Técnicas ---
  tam_fatias       = 12,        # Nomes das modalidades nas fatias do gráfico
  cor_eficiencia   = "#0b132b", # Azul Marinho para a Eficiência
