...
tabela_interativa_detalhes <- function(dados, 
  ...
  # Adiciona a coluna do botão [+] na primeira posição
  dados_processados <- base::cbind(' ' = '&oplus;', dados_processados)

  # Contador automático de títulos
  novo_valor <- base::get("contador_tabelas_alex", envir = .GlobalEnv) + 1
  base::assign("contador_tabelas_alex", novo_valor, envir = .GlobalEnv)
  titulo_final <- base::paste0("Tabela ", novo_valor, ": ", titulo_tabela)

  # Título formatado em HTML
  html_titulo <- htmltools::tags$h2(
    style = base::paste0("color: ", cor_titulo, "; font-size: ", tam_fonte_titulo, 
                         "; font-weight: bold; text-align: left; margin-bottom: 10px;"),
  ...
  # --- 2. PREPARAÇÃO JSON PARA JAVASCRIPT ---
  # Transformamos os dados de detalhes em JSON para o navegador processar o clique
  ...
  # --- 3. CONFIGURAÇÃO DE LARGURAS DA TABELA PAI ---
  # Mapeia as larguras para o formato do DataTables
  ...
  if (!base::is.null(largura_cols_vetor)) {
    for (i in 1:base::length(largura_cols_vetor)) {
      lista_column_defs[[i + 1]] <- base::list(targets = i, width = largura_cols_vetor[i])
    }
  }
    ...
    options = base::list(
      ...
      initComplete = DT::JS(
        base::paste0("function(settings, json) {",
                     ...
                     # Variáveis injetadas
                     "var table = this.api();",
                     "var detalhes_data = ", json_detalhes_js, ";",
                     ...
                     "    tr.css({'background-color': '#f8f9fa', 'color': '", cor_primaria, "', 'font-weight': '600', 'border-left': '4px solid ", cor_primaria, "'});",
                     "    var rowData = row.data();",
                     "    var indexChave = ", if(mostrar_indice) "2" else "1", ";", 
                     "    var chave = rowData[indexChave];",
                     "    var sub_dados = detalhes_data.filter(d => d['", coluna_chave, "'] == chave);",
                     ...
                     "          colKeys.forEach(k => { html_sub += '<td style=\"padding:6px; color:#333 !important;\">' + (r[k] || '') + '</td>'; });",
                     "          html_sub += '</tr>';",
                     "       });",
...
plotly::plot_ly() %>%
  ...
  plotly::layout(
    ...
    margin = base::list(t = 150, b = 100, l = 10, r = 10),
...
tabela_interativa_detalhes(
  ...
  mostrar_filtro         = FALSE,                 # Ativa busca por coluna (limpando a col do [+])
