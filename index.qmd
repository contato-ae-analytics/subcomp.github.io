...
tabela_interativa <- function(dados, 
  ...
  if (!base::is.null(largura_cols_vetor)) {
    for (i in base::seq_along(largura_cols_vetor)) {
      if (i < n_colunas_dados) { # Trava para evitar o erro de parâmetro desconhecido
  ...
  contador <- base::get("contador_tabelas_alex", envir = .GlobalEnv) + 1
  base::assign("contador_tabelas_alex", contador, envir = .GlobalEnv)
  titulo_final <- base::paste0("Tabela ", contador, ": ", titulo_tabela)
  ...
  tabela <- DT::datatable(
    ...
    options = base::list(
      dom = 'f t p', # 'f' na esquerda como era antes
      pageLength = n_pages,
      searching = TRUE, 
      lengthChange = FALSE,
      info = FALSE,
      ...
      initComplete = DT::JS(
        base::paste0("function(settings, json) {",
                     ...
                     # HOVER ORIGINAL DO ALEX: Fundo cinza + Negrito + Texto Preto
                     "$(this.api().table().body()).on('mouseenter', 'tr', function() {",
                     "  $(this).css({'background-color': '#f1f1f1', 'color': '#000000', 'font-weight': 'bold', 'transition': '0.1s'});",
...
tabela_interativa_detalhes <- function(dados, 
  ...
  tabela <- DT::datatable(
    ...
    class = "row-border", 
    options = base::list(
      dom = 'f t p', # Barra de pesquisa na esquerda
      pageLength = n_pages,
      searching = TRUE, 
      lengthChange = FALSE,
      info = FALSE,
      ...
      initComplete = DT::JS(
        base::paste0("function(settings, json) {",
                     ...
                     "    tr.css({'background-color': '#f1f1f1', 'color': '#000000', 'font-weight': 'bold', 'transition': '0.1s'});",
                     "  }",
                     "}).on('mouseleave', 'tr', function() {",
                     "  var tr = $(this);",
                     "  if (!tr.hasClass('shown') && !tr.closest('table').hasClass('subtable')) {",
...
```{r}
# ==========================================================
# 1. PREPARAÇÃO DA BASE DE DADOS PARA DOWNLOAD
# ==========================================================

# Criamos o dataframe com os nomes amigáveis, descrições e links reais fornecidos
bd_central_downloads <- base::data.frame(
  ...
  "descricao_bd" = base::c(
    "Cabeçalho da Operação (Datas, Setores, Agentes)",
...
# Criando a coluna com o botão de acesso (HTML) no estilo azul oficial
bd_central_downloads$botao_acesso <- base::apply(bd_central_downloads, 1, function(linha) {
  base::paste0(
    "<a href='", linha['url_original'], "' target='_blank' ",
    "style='background-color:#03045e; color:white; padding:4px 12px; ",
    "text-decoration:none; border-radius:4px; font-weight:bold; font-size:10px;'>",
    "PLANILHA</a>"
  )
})

# --- VETORES DE CONFIGURAÇÃO ALEX ---
minhas_colunas_downloads <- base::c(
  "Banco de Dados" = "id_planilha", 
  "Descrição do Conteúdo" = "descricao_bd", 
  "Acesso Direto" = "botao_acesso"
)

minhas_larguras_downloads <- base::c("180px", "450px", "120px")


# ==========================================================
# 2. EXECUÇÃO: TABELA CENTRAL DE DOWNLOADS
# ==========================================================

tabela_interativa(
  dados = bd_central_downloads,
  titulo_tabela = "Repositório e Central de Downloads",
  
  # Configurações Visuais (Visual Compacto Alex)
  ...
  # Parâmetros de Layout e Filtro
  mostrar_filtro   = FALSE,
  pesquisa_geral   = TRUE,
  n_pages          = 6,
  mostrar_indice   = FALSE,
  
  # Aplicação de Vetores e Seleção
  largura_cols_vetor   = minhas_larguras_downloads,
  colunas_selecionadas = minhas_colunas_downloads
)
```
