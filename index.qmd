...
# ==========================================================
# --- FUNÇÃO: TABELA INTERATIVA COM DESTAQUE ELEGANTE ---
# ==========================================================
    ...
    options = base::list(
      ...
      initComplete = DT::JS(
        base::paste0("function(settings, json) {",
                     ...
                     # Lógica de expansão com destaque ELEGANTE
                     "var table = this.api(); var detalhes_data = ", json_detalhes, ";",
                     "var customNomes = ", nomes_detalhes_js, "; var customLarguras = ", largura_detalhes_js, ";",
                     ...
                     "  } else {",
                     # DESTAQUE REFINADO (Fundo claro + Negrito moderado + Borda lateral)
                     ...
                     "    html_sub += '</tr></thead><tbody>';",
                     "    sub_dados.forEach(r => {",
                     "       html_sub += '<tr style=\"border-bottom:1px solid #f1f1f1; color:#333 !important;\">';",
                     "       colKeys.forEach(k => { html_sub += '<td style=\"padding:6px;\">' + r[k] + '</td>'; });",
                     "       html_sub += '</tr>';",
                     "    });",
                     "    html_sub += '</tbody></table>';",
                     "    row.child(html_sub).show(); tr.addClass('shown'); $(this).html('&ominus;');",
                     ...
                     "$(this.api().table().body()).on('mouseenter', 'tr', function() {",
                     "  if (!$(this).hasClass('shown') && !$(this).closest('table').hasClass('subtable')) {",
                     "    $(this).css({'background-color': '#f1f1f1', 'transition': '0.1s'});",
                     "  }",
                     "}).on('mouseleave', 'tr', function() {",
                     "  if (!$(this).hasClass('shown') && !$(this).closest('table').hasClass('subtable')) {",
