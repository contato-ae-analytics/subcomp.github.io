...
```{mermaid}
graph TD
    classDef lider fill:#03045e,color:#fff,stroke:#000,stroke-width:2px,font-weight:bold;
    classDef equipe fill:#48cae4,color:#000,stroke:#03045e,stroke-width:1px;
    classDef estagio fill:#caf0f8,color:#000,stroke:#03045e,stroke-dasharray: 5 5;

    SG("Subgerente<br/>(Rodrigo Ferreira Andrade)"):::lider
    SG --> AA1("Assistente Administrativo<br/>(Daniel Andrade)"):::equipe
    SG --> AA2("Assistente Administrativo<br/>(José Victor)"):::equipe
    SG --> ES("Estagiária I<br/>(Fernanda Viana)"):::estagio
```
...
<div style="font-family: 'Inter', sans-serif; max-width: 500px; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.1); border: 1px solid #e0e0e0;">
    <div style="background-color: #03045e; color: white; padding: 15px; text-align: center; font-weight: bold; font-size: 1.1em;">
        Equipe de Gestão - Compras 2025
    </div>
    <table style="width: 100%; border-collapse: collapse; background: white;">
        <tr style="border-bottom: 1px solid #eee;">
            <td style="padding: 12px 20px; color: #666; font-size: 0.9em; font-weight: bold;">Subgerente</td>
            <td style="padding: 12px 20px; color: #03045e; font-weight: bold;">Rodrigo Ferreira Andrade</td>
        </tr>
        <tr style="border-bottom: 1px solid #eee;">
            <td style="padding: 12px 20px; color: #666; font-size: 0.9em;">Assistente Adm.</td>
            <td style="padding: 12px 20px;">Daniel Andrade</td>
        </tr>
        <tr style="border-bottom: 1px solid #eee;">
            <td style="padding: 12px 20px; color: #666; font-size: 0.9em;">Assistente Adm.</td>
            <td style="padding: 12px 20px;">José Victor</td>
        </tr>
        <tr>
            <td style="padding: 12px 20px; color: #666; font-size: 0.9em;">Estagiária I</td>
            <td style="padding: 12px 20px;">Fernanda Viana</td>
        </tr>
    </table>
</div>
```
...
#     
#     /* DEIXA OS BOTÕES ARREDONDADOS */
#     .updatemenu-container .updatemenu-header, 
#     .updatemenu-container .updatemenu-item {
#       border-radius: 12px !important; /* Ajuste o número para ficar mais ou menos redondo */
#       border: 1px solid #D3D3D3 !important;
#       margin-bottom: 2px !important;
#     }
# 
#     /* Ajuste para o fundo do botão quando passar o mouse */
...
# # --- 3. DADOS ---
# meses_ordem <- c("<b>Janeiro</b>", "<b>Fevereiro</b>", "<b>Março</b>", "<b>Abril</b>", 
#                  "<b>Maio</b>", "<b>Junho</b>", "<b>Julho</b>", "<b>Agosto</b>", 
#                  "<b>Setembro</b>", "<b>Outubro</b>", "<b>Novembro</b>", "<b>Dezembro</b>")
# 
...
#   dplyr::distinct(processo, .keep_all = TRUE) %>%
#   dplyr::select(processo, mes_entrada, mes_saida, modalidade_processo, status_item) %>%
#   tidyr::pivot_longer(cols = c(mes_entrada, mes_saida), names_to = "Origem", values_to = "Mes_Nome") %>%
#   dplyr::mutate(Tipo = dplyr::if_else(Origem == "mes_entrada", "Entradas", "Saídas")) %>%
#   dplyr::filter(!is.na(Mes_Nome)) %>%
#   dplyr::mutate(Mes_Bold = factor(paste0("<b>", stringr::str_to_title(as.character(Mes_Nome)), "</b>"), levels = meses_ordem))
...
# esqueleto <- expand.grid(Mes_Bold = factor(meses_ordem, levels = meses_ordem), Tipo = c("Entradas", "Saídas"), stringsAsFactors = FALSE)
...
#     dplyr::group_by(Mes_Bold, Tipo) %>%
#     dplyr::summarise(n = dplyr::n(), .groups = "drop") %>%
#     # Join exato: O fator Mes_Bold da base encontra o fator Mes_Bold do esqueleto
#     dplyr::right_join(esqueleto, by = c("Mes_Bold", "Tipo")) %>%
#     dplyr::mutate(
#       Quantidade = tidyr::replace_na(n, 0),
#       Hover = ifelse(Quantidade > 0, paste0("Total: ", Quantidade), "Sem registros")
#     ) %>%
#     dplyr::arrange(Tipo, Mes_Bold)
#   
#   return(df_res)
...
#   
#   df_res <- df %>%
#     # LIMPEZA CRUCIAL: Remove espaços e garante formato "Janeiro"
#     dplyr::mutate(Mes_Nome = stringr::str_trim(stringr::str_to_title(as.character(Mes_Nome)))) %>%
#     dplyr::group_by(Mes_Nome, Tipo) %>%
#     dplyr::summarise(n = dplyr::n(), .groups = "drop") %>%
#     # Monta o label exatamente como o Plotly espera
#     dplyr::mutate(Mes_Bold = factor(paste0("<b>", Mes_Nome, "</b>"), levels = meses_ordem)) %>%
#     dplyr::right_join(esqueleto, by = c("Mes_Bold", "Tipo")) %>%
#     dplyr::mutate(
#       Quantidade = tidyr::replace_na(n, 0),
#       Hover = ifelse(Quantidade > 0, paste0("Total: ", Quantidade), "Sem registros")
...
#     dplyr::summarise(n = dplyr::n(), .groups = "drop") %>%
#     dplyr::group_by(Mes_Bold, Tipo) %>%
#     dplyr::mutate(total = sum(n), txt_h = paste0("• ", modalidade_processo, ": ", n, " (", round((n/total)*100,1), "%)")) %>%
#     dplyr::summarise(Quantidade = max(total), Hover = paste(txt_h, collapse = "<br>"), .groups = "drop") %>%
#     dplyr::right_join(esqueleto, by = c("Mes_Bold", "Tipo")) %>%
#     dplyr::mutate(Quantidade = ifelse(is.infinite(Quantidade) | is.na(Quantidade), 0, Quantidade), 
#                   Hover = ifelse(is.na(Hover), "Sem processos registrados", Hover)) %>%
#     dplyr::arrange(Tipo, Mes_Bold)
#   return(df_res)
# }
...
df_e_ini <- df_i %>% dplyr::filter(Tipo == "Entradas") ; print(df_e_ini)
df_s_ini <- df_i %>% dplyr::filter(Tipo == "Saídas") ; print(df_s_ini)
...
plotly::plot_ly() %>%
  ...
  plotly::layout(
    ...
    # xaxis = list(title = list(text = "<br><br><br>", font = list(size = 1)),
    #              showgrid = FALSE, 
    #              tickfont = list(family = "Inter, sans-serif", size = tamanho_eixo_x, color = cor_eixo_x)
    #              ),
    yaxis = list(title = "", 
    ...
    #        xanchor = "left",
    #        buttons = list(
    #          list(method = "restyle", label = "Gráfico: Barras", 
    #               args = list(list(type = "bar", textposition = "outside"))),
    #          list(method = "restyle", label = "Gráfico: Linhas", 
    #               args = list(list(type = "scatter", mode = "lines+markers+text", textposition = "top center")))), 
    #        # bgcolor = "#F8F9FA", bordercolor = "#D3D3D3", borderwidth = 1,
    #        font = list(family = "Inter, sans-serif", size = tamanho_botoes))
    # ),
    ...
    margin = list(
      t = 140,  # Espaço para Título + Subtítulo + Legenda + 3 Botões
...
# --- 1. DADOS "ANTES" (BRUTOS DO PIVOT) ---
# Aqui vemos se o R conseguiu extrair as datas do Excel corretamente
dados_antes_plotly <- dados_base_limpa %>%
  dplyr::group_by(Mes_Nome, Tipo) %>%
  dplyr::summarise(Total_Real = dplyr::n(), .groups = "drop")

# --- 2. DADOS "DEPOIS" (PROCESSADOS PELA FUNÇÃO) ---
# Aqui vemos se o 'right_join' ou os 'factors' zeraram os dados
dados_depois_plotly <- gerar_dados_seguros(dados_base_limpa) %>%
  dplyr::select(Mes_Bold, Tipo, Quantidade)

# --- 3. EXIBIÇÃO: TABELA ANTES ---
tabela_interativa(
  dados = dados_antes_plotly,
  titulo_tabela = "AUDITORIA 1: Dados Brutos Extraídos (Antes do Esqueleto)",
  tam_fonte_corpo = "11px",
  cor_titulo = "#0077b6", # Azul para o 'Antes'
  largura_cols_vetor = c("30px", "200px", "150px", "150px"),
  colunas_selecionadas = c("Mês" = "Mes_Nome", "Fluxo" = "Tipo", "Contagem" = "Total_Real")
)

# --- 4. EXIBIÇÃO: TABELA DEPOIS ---
tabela_interativa(
  dados = dados_depois_plotly,
  titulo_tabela = "AUDITORIA 2: Dados Finais Enviados ao Plotly (Depois do Esqueleto)",
  tam_fonte_corpo = "11px",
  cor_titulo = "#d62828", # Vermelho para o 'Depois' (Onde os zeros aparecem)
  largura_cols_vetor = c("30px", "200px", "150px", "150px"),
  colunas_selecionadas = c("Mês HTML" = "Mes_Bold", "Fluxo" = "Tipo", "Quantidade" = "Quantidade")
)
```
...
# ==========================================================
# --- 1. ÁREA DE PERSONALIZAÇÃO (PAINEL DE CONTROLE) ---
# ==========================================================
...
# ==========================================================
# --- 2. TRATAMENTO DE DADOS E DICIONÁRIO ---
# ==========================================================

meses_ordem_limpo <- c("Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", 
                       "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro")

# Tradutor para garantir que meses em Inglês (GitHub) virem Português
tradutor_meses <- c(
  "January" = "Janeiro", "February" = "Fevereiro", "March" = "Março", 
  "April" = "Abril", "May" = "Maio", "June" = "Junho", 
  "July" = "Julho", "August" = "Agosto", "September" = "Setembro", 
...
plotly::plot_ly() %>%
  ...
  plotly::layout(
    ...
    hoverlabel = list(
      ...
      font = list(family = "Inter, sans-serif", size = hover_fonte_tam, color = hover_txt_cor),
      bordercolor = borda_balao_hover, 
      align = alinhamento_balao_hover
    ),
    xaxis = list(title = "", showgrid = FALSE, tickfont = list(size = tamanho_eixo_x, color = cor_eixo_x), fixedrange = TRUE),
    yaxis = list(title = "", showticklabels = FALSE, showgrid = TRUE, gridcolor = "#F0F0F0", 
                 range = list(0, max(df_i$Quantidade) * 1.5)),
    barmode = 'group',
    bargap = 0.2, 
    bargroupgap = 0.1,
    updatemenus = list(
      list(active = 0, x = 0, y = 1.13, xanchor = "left", buttons = montar_botoes(lista_m, "mod"),
           font = list(family = "Inter, sans-serif", size = tamanho_botoes)),
      list(active = 0, x = 0.30, y = 1.13, xanchor = "left", buttons = montar_botoes(lista_s, "stat"),
           font = list(family = "Inter, sans-serif", size = tamanho_botoes)),
      list(active = 0, x = 0.55, y = 1.13, xanchor = "left",
           buttons = list(
             list(method = "update", label = "Gráfico: Barras", args = list(list(type = "bar", textposition = "outside"))),
             list(method = "update", label = "Gráfico: Linhas", args = list(list(type = "scatter", mode = "lines+markers+text", textposition = "top center")))
           ), font = list(family = "Inter, sans-serif", size = tamanho_botoes))
    ),
    legend = list(orientation = 'h', x = 0, xanchor = "left", y = 1.25,
                  font = list(family = "Inter, sans-serif", size = tamanho_legenda, color = cor_legenda)),
    margin = list(t = 140, b = 170, l = 30, r = 30),
    plot_bgcolor = "white", paper_bgcolor = "white"
  ) %>%
  plotly::config(displayModeBar = FALSE, responsive = TRUE)
