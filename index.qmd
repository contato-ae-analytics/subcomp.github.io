...
gerar_grafico_natureza_top <- function(bd_principal, 
  ...
  plotly::plot_ly(df_init) %>%
    ...
    plotly::layout(
      ...
      margin = base::list(t = 20, b = 20, l = 320),
...
```{r}
# ==========================================================
# 1. PRÉ-PROCESSAMENTO: STATUS POR PROCESSO (BANCO PAI)
# ==========================================================
status_por_processo <- bd_principal %>%
  dplyr::group_by(processo) %>%
  dplyr::summarise(
  ...
  dplyr::filter(!base::is.na(mod_proc))
...
bd_total_geral_pai <- bd_pai_modalidade_base %>%
  dplyr::summarise(
    mod_proc         = "TOTAL GERAL",
    n_processos      = base::sum(n_processos),
    v_total_mod      = base::sum(v_total_mod),
...
bd_pai_modalidade <- base::rbind(bd_total_geral_pai, bd_pai_modalidade_base) %>%
  dplyr::mutate(
    p_represent_txt  = base::paste0(base::round((n_processos / base::max(n_processos[mod_proc == "TOTAL GERAL"])) * 100, 1), "%"),
    aporte_total     = base::paste0("R$ ", base::format(v_total_mod, big.mark = ".", decimal.mark = ",")),
    aporte_resolvido = base::paste0("R$ ", base::format(v_resolvido, big.mark = ".", decimal.mark = ",")),
...
# ==========================================================
# 2. BANCO FILHO: NATUREZAS COM VALORES E FREQUÊNCIA
# ==========================================================
quebrar_texto_tab <- function(x, n = 40) {
  base::lapply(x, function(s) {
    base::paste(base::strwrap(s, width = n), collapse = "<br>")
  }) %>% base::unlist()
}

# Função de processamento corrigida para evitar erro de tamanho (p_impacto)
processar_filho_natureza <- function(df_input, label_mod, v_referencia) {
  # Garante que v_referencia seja um único número (primeira posição)
  v_ref <- v_referencia[1]
  if(base::is.na(v_ref) || v_ref == 0) v_ref <- 1
  ...
  df_input %>%
    ...
    dplyr::summarise(
      ...
      v_res_nat       = base::sum(base::ifelse(status_resumo == "RES", valor_total, 0), na.rm = TRUE),
      v_pen_nat       = base::sum(base::ifelse(status_resumo == "PEN", valor_total, 0), na.rm = TRUE),
      .groups = "drop"
    ) %>%
    dplyr::mutate(
      natureza_rank   = base::paste0("<b>", natureza_despesa, "</b> - ", quebrar_texto_tab(desc_nat)),
      p_impacto       = base::paste0(base::round((v_total_nat / v_ref) * 100, 1), "%"),
      total_txt       = base::paste0("R$ ", base::format(v_total_nat, big.mark = ".", decimal.mark = ",")),
      empenhado_txt   = base::paste0("R$ ", base::format(v_res_nat, big.mark = ".", decimal.mark = ",")),
      pendente_txt    = base::paste0("R$ ", base::format(v_pen_nat, big.mark = ".", decimal.mark = ","))
    )
}

# Naturezas por Modalidade
bd_filho_naturezas <- bd_principal %>%
  dplyr::group_split(modalidade_processo) %>%
  base::lapply(function(sub) {
    mod_nome <- base::as.character(base::unique(sub$modalidade_processo))
    # Busca o valor da modalidade no pai e garante que pegue apenas um valor
    v_mod <- bd_pai_modalidade$v_total_mod[bd_pai_modalidade$mod_proc == mod_nome]
    processar_filho_natureza(sub, mod_nome, v_mod)
  }) %>%
  dplyr::bind_rows()

# Naturezas para o Total Geral
v_geral_total <- bd_pai_modalidade$v_total_mod[bd_pai_modalidade$mod_proc == "TOTAL GERAL"]
detalhes_total_geral <- processar_filho_natureza(bd_principal, "TOTAL GERAL", v_geral_total)

bd_filho_final <- dplyr::bind_rows(bd_filho_naturezas, detalhes_total_geral) %>%
  dplyr::select(mod_proc, natureza_rank, frequencia, p_impacto, total_txt, empenhado_txt, pendente_txt)
...
nomes_col <- base::c(
  ...
  "Part. (%)"              = "p_represent_txt", 
  "Valor Total"            = "aporte_total",
  "Qtd. Empenhados"        = "n_resolvidos",
  "Valor Empenhado"        = "aporte_resolvido",
  "Qtd. Pendentes"         = "n_pendentes",
  "Valor Pendente"         = "aporte_pendente"
)

nomes_det_alex <- base::c(
  "Natureza de Despesa", 
  "Pedidos (Freq.)",
  "Impacto na Mod. (%)", 
  "Valor Total (R$)",
...
tabela_interativa_detalhes(
  ...
  coluna_chave               = "mod_proc",
  titulo_tabela              = "Detalhamento: Modalidade x Natureza de Despesa",
  tam_fonte_corpo            = 12,
  tam_fonte_header           = 13,
  cor_titulo                 = "#03045e",
  cor_primaria               = "#001d3d",
  colunas_selecionadas       = nomes_col,
  colunas_detalhes_nomes     = nomes_det_alex,
  largura_cols_vetor         = base::c("150px", "80px", "80px", "180px", "80px", "180px", "80px", "180px"),
  largura_detalhes_vetor     = base::c("350px", "90px", "110px", "200px", "200px", "200px"),
  pesquisa_geral             = TRUE
)
```
